...
class ProductTest < ActiveSupport::TestCase
  ...
  test 'should calculate maturity_scores' do
    ...
    maturity_scores = calculate_maturity_scores(product2.id, rubric.id)[:rubric_scores]
    ...
    maturity_scores = calculate_maturity_scores(product3.id, rubric.id)[:rubric_scores]
                      .first[:category_scores]
                      .first[:indicator_scores]
