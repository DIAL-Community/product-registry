...
class CreatePlaybooks < ActiveRecord::Migration[5.2]
  def change
    ...
    create_table(:tasks_organizations) do |t|
      ...
      t.index(%w[organization_id task_id], name: 'organizations_plays_idx', unique: true)
      t.index(%w[task_id organization_id], name: 'plays_organizations_idx', unique: true)
    end
    ...
    create_table(:tasks_products) do |t|
      ...
      t.index(%w[product_id task_id], name: 'products_tasks_idx', unique: true)
      t.index(%w[task_id product_id], name: 'tasks_products_idx', unique: true)
    end
    ...
    create_table(:plays_tasks) do |t|
      ...
      t.bigint('task_id', null: false)
      t.index(%w[task_id play_id], name: 'tasks_plays_idx', unique: true)
      t.index(%w[play_id task_id], name: 'plays_tasks_idx', unique: true)
    end
    ...
    create_table(:activities_principles) do |t|
      ...
      t.index(%w[digital_principle_id activity_id], name: 'principles_activities_idx', unique: true)
      t.index(%w[activity_id digital_principle_id], name: 'activities_principles_idx', unique: true)
    end
    ...
    create_table(:activities_tasks) do |t|
      t.bigint(:activity_id, null: false)
      t.bigint(:task_id, null: false)
      t.index(%w[task_id activity_id], name: 'tasks_activities_idx', unique: true)
      t.index(%w[activity_id task_id], name: 'activities_tasks_idx', unique: true)
    end
