connection = ActiveRecord::Base.connection()

Sector.create(name: "Education", slug: "education")
Sector.create(name: "Economics", slug: "economics")
Sector.create(name: "Food Security", slug: "food_security")
Sector.create(name: "Agircutlure", slug: "agircutlure")
Sector.create(name: "Data Security", slug: "data_security")
Sector.create(name: "Health", slug: "health")
Sector.create(name: "Justice", slug: "justice")
Sector.create(name: "Governance", slug: "governance")
Sector.create(name: "ICT4D", slug: "ictd")
Sector.create(name: "Platform creation", slug: "platform_creation")
Sector.create(name: "Information Systems", slug: "information_systems")
Sector.create(name: "Agriculture", slug: "agriculture")
Sector.create(name: "Communication", slug: "communication")
Sector.create(name: "Gender", slug: "gender")
Sector.create(name: "Finance", slug: "finance")
Sector.create(name: "Employment", slug: "employment")
Sector.create(name: "Energy", slug: "energy")
Sector.create(name: "Environemnt", slug: "environemnt")
Sector.create(name: "Water", slug: "water")
Sector.create(name: "Finance and Insurance", slug: "finance_and_insurance")
Sector.create(name: "Media", slug: "media")
Sector.create(name: "Entertainment", slug: "entertainment")
Sector.create(name: "Travel and Hospitality", slug: "travel_and_hospitality")
Sector.create(name: "Membership Associations", slug: "membership_associations")
Sector.create(name: "Higher Education", slug: "higher_education")
Sector.create(name: "Research collaborations", slug: "research_collaborations")
Sector.create(name: "Engineering", slug: "engineering")
Sector.create(name: "NGO", slug: "ngo")
Sector.create(name: "Government", slug: "government")
Sector.create(name: "Emergency", slug: "emergency")
Sector.create(name: "Microfinance", slug: "microfinance")
Sector.create(name: "Justice & Peacebuilding", slug: "justice__peacebuilding")
Sector.create(name: "Youth", slug: "youth")
Sector.create(name: "ANTI-CORRUPTION & ETHICS", slug: "anticorruption__ethics")
Sector.create(name: "ENTERPRISE ECOSYSTEMS", slug: "enterprise_ecosystems")
Sector.create(name: "BUSINESS ADVOCACY", slug: "business_advocacy")
Sector.create(name: "DEMOCRATIC GOVERNANCE", slug: "democratic_governance")
Sector.create(name: "TRADE", slug: "trade")
Sector.create(name: "CENTERS OF EXCELLENCE", slug: "centers_of_excellence")
Sector.create(name: "food", slug: "food")
Sector.create(name: "income & local markets", slug: "income__local_markets")
Sector.create(name: "financial", slug: "financial")
Sector.create(name: "management", slug: "management")
Sector.create(name: "protection", slug: "protection")
Sector.create(name: "Agriculture and Food Security", slug: "agriculture_and_food_security")
Sector.create(name: "Corporate Partnerships", slug: "corporate_partnerships")
Sector.create(name: "Democracy and Governance", slug: "democracy_and_governance")
Sector.create(name: "Economic Growth and Trade", slug: "economic_growth_and_trade")
Sector.create(name: "Education and Youth", slug: "education_and_youth")
Sector.create(name: "Environment and Natural Resources", slug: "environment_and_natural_resource")
Sector.create(name: "Gender Equality and Social Inclusion", slug: "gender_equality_and_social_inclu")
Sector.create(name: "Peace", slug: "peace")
Sector.create(name: "Stability", slug: "stability")
Sector.create(name: "and Transition", slug: "and_transition")
Sector.create(name: "Supply Chain Solutions", slug: "supply_chain_solutions")
Sector.create(name: "and Sustainable Cities", slug: "and_sustainable_cities")
Sector.create(name: "Capacity Development", slug: "capacity_development")
Sector.create(name: "Citizen Security", slug: "citizen_security")
Sector.create(name: "CVE and PVE", slug: "cve_and_pve")
Sector.create(name: "Digital Development", slug: "digital_development")
Sector.create(name: "Education In Conflict", slug: "education_in_conflict")
Sector.create(name: "Elections", slug: "elections")
Sector.create(name: "Instructional Systems", slug: "instructional_systems")
Sector.create(name: "Livelihoods", slug: "livelihoods")
Sector.create(name: "Political Transitions", slug: "political_transitions")
Sector.create(name: "Stabilization", slug: "stabilization")
Sector.create(name: "Workforce", slug: "workforce")
Sector.create(name: "Digital Acceleration", slug: "digital_acceleration")
Sector.create(name: "Economic Growth", slug: "economic_growth")
Sector.create(name: "Environment & Energy", slug: "environment__energy")
Sector.create(name: "Fragile States", slug: "fragile_states")
Sector.create(name: "Global Health", slug: "global_health")
Sector.create(name: "Sustainable Business", slug: "sustainable_business")
Sector.create(name: "Data Management & Policy", slug: "data_management__policy")
Sector.create(name: "Health - Consultancy", slug: "health__consultancy")
Sector.create(name: "Agriculture/Poverty", slug: "agriculturepoverty")
Sector.create(name: "Communications", slug: "communications")
Sector.create(name: "Agriculture & Rural Development", slug: "agriculture__rural_development")
Sector.create(name: "Digitalization", slug: "digitalization")
Sector.create(name: "Training & Employment", slug: "training__employment")
Sector.create(name: "Environment & Climate", slug: "environment__climate")
Sector.create(name: "Water & Sanitation", slug: "water__sanitation")
Sector.create(name: "Civil society", slug: "civil_society")
Sector.create(name: "Communication and social marketing", slug: "communication_and_social_marketi")
Sector.create(name: "Crisis response", slug: "crisis_response")
Sector.create(name: "Economic development", slug: "economic_development")
Sector.create(name: "Environment", slug: "environment")
Sector.create(name: "Nutrition", slug: "nutrition")
Sector.create(name: "Research", slug: "research")
Sector.create(name: "Technology", slug: "technology")
Sector.create(name: "Humanitarian - Poverty", slug: "humanitarian__poverty")
Sector.create(name: "hunger", slug: "hunger")
Sector.create(name: "Humanitarian - Emergency?", slug: "humanitarian__emergency")
Sector.create(name: "Socio-economic", slug: "socioeconomic")
Sector.create(name: "Poverty", slug: "poverty")
Sector.create(name: "Media Production", slug: "media_production")
Sector.create(name: "Consulting", slug: "consulting")
Sector.create(name: "Climate", slug: "climate")
Sector.create(name: "Humanitarian", slug: "humanitarian")
Sector.create(name: "Governance ?", slug: "governance_")
Sector.create(name: "Work Force", slug: "work_force")
Sector.create(name: "Logisitcs", slug: "logisitcs")
Sector.create(name: "Humanitarian- Emergency", slug: "humanitarian_emergency")
Sector.create(name: "Workforce development", slug: "workforce_development")
Sector.create(name: "Food security and agriculture", slug: "food_security_and_agriculture")
Sector.create(name: "social and justice poilcy", slug: "social_and_justice_poilcy")
Sector.create(name: "innovation ecosystem", slug: "innovation_ecosystem")
Sector.create(name: "Conflict Sensativity", slug: "conflict_sensativity")
Sector.create(name: "Natural Resource Conflicts", slug: "natural_resource_conflicts")
Sector.create(name: "Disarmament", slug: "disarmament")
Sector.create(name: "Demobilization & Reintegration", slug: "demobilization__reintegration")
Sector.create(name: "Reconciliation", slug: "reconciliation")
Sector.create(name: "Fair & Responsibile Media", slug: "fair__responsibile_media")
Sector.create(name: "Refugee & Internal Displacement Issues", slug: "refugee__internal_displacement_i")
Sector.create(name: "Gender Equality", slug: "gender_equality")
Sector.create(name: "Security Sector Reform", slug: "security_sector_reform")
Sector.create(name: "Democracy & Elections", slug: "democracy__elections")
Sector.create(name: "Sexual & Gender Based Violence", slug: "sexual__gender_based_violence")
Sector.create(name: "Religious Engagement", slug: "religious_engagement")
Sector.create(name: "Violent Exteremism", slug: "violent_exteremism")
Sector.create(name: "Education & Youth", slug: "education__youth")
Sector.create(name: "Democracy & Governance", slug: "democracy__governance")
Sector.create(name: "Economic Growth & Employment", slug: "economic_growth__employment")
Sector.create(name: "Emergency Response", slug: "emergency_response")
Sector.create(name: "Women & Gender Equality", slug: "women__gender_equality")
Sector.create(name: "Data Collection", slug: "data_collection")
Sector.create(name: "Humaitarian Aid", slug: "humaitarian_aid")
Sector.create(name: "Human rights", slug: "human_rights")
Sector.create(name: "Helath", slug: "helath")
Sector.create(name: "Market Development", slug: "market_development")
Sector.create(name: "Sustainable social development", slug: "sustainable_social_development")
Sector.create(name: "Conflict resolution & Peace & Security", slug: "conflict_resolution__peace__secu")
Sector.create(name: "Sanitation", slug: "sanitation")
Sector.create(name: "Education - training", slug: "education__training")
Sector.create(name: "Infrastructure", slug: "infrastructure")
Sector.create(name: "Resource Marangement", slug: "resource_marangement")
Sector.create(name: "Transparency & Accountability", slug: "transparency__accountability")
Sector.create(name: "Software", slug: "software")
Sector.create(name: "Environment - Cllimate Change", slug: "environment__cllimate_change")
Sector.create(name: "DEVELOPMENT IMPACT", slug: "development_impact")
Sector.create(name: "Energy & Climate", slug: "energy__climate")
Sector.create(name: "Big Data", slug: "big_data")
Sector.create(name: "Humaitarian", slug: "humaitarian")
Sector.create(name: "Adolescent and Youth Female Gential Mutilation Emergencies World Population Supplies Midwifery", slug: "adolescent_and_youth_female_gent")
Sector.create(name: "Democracy", slug: "democracy")
Sector.create(name: "Human Rights and Governance", slug: "human_rights_and_governance")
Sector.create(name: "Environment and Global Climate Change", slug: "environment_and_global_climate_c")
Sector.create(name: "Gender Equality and Women's Empowerment", slug: "gender_equality_and_womens_empow")
Sector.create(name: "Water and Sanitation", slug: "water_and_sanitation")
Sector.create(name: "Working in Crises and Conflict", slug: "working_in_crises_and_conflict")
Sector.create(name: "Finacial Services", slug: "finacial_services")
Sector.create(name: "Economic", slug: "economic")
Sector.create(name: "Advocacy", slug: "advocacy")
Sector.create(name: "Refugees", slug: "refugees")
Sector.create(name: "Food Assistance", slug: "food_assistance")

o = Organization.create(name: "Abt Associates", slug: "abt_associates", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://abtassociates.com/")

o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'economics').limit(1)[0]
o.sectors << Sector.where(slug: 'food_security').limit(1)[0]
o.sectors << Sector.where(slug: 'agircutlure').limit(1)[0]
o.sectors << Sector.where(slug: 'data_security').limit(1)[0]
o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'justice').limit(1)[0]
o.sectors << Sector.where(slug: 'governance').limit(1)[0]
o.locations << Location.create(name: "Australia", slug: 'australia', :location_type => :country)
o.locations << Location.create(name: "Cambodia", slug: 'cambodia', :location_type => :country)
o.locations << Location.create(name: "Indonesia", slug: 'indonesia', :location_type => :country)
o.locations << Location.create(name: "Laos", slug: 'laos', :location_type => :country)
o.locations << Location.create(name: "Papua New Guinea", slug: 'papua_new_guinea', :location_type => :country)
o.locations << Location.create(name: "Philippines", slug: 'philippines', :location_type => :country)
o.locations << Location.create(name: "Thailand", slug: 'thailand', :location_type => :country)
o.locations << Location.create(name: "Timor-Leste", slug: 'timorleste', :location_type => :country)
o.locations << Location.create(name: "Vietnam", slug: 'vietnam', :location_type => :country)
o.locations << Location.create(name: "United Kingdom", slug: 'united_kingdom', :location_type => :country)
o.locations << Location.create(name: "Dominican Republic", slug: 'dominican_republic', :location_type => :country)
o.locations << Location.create(name: "El Salvador", slug: 'el_salvador', :location_type => :country)
o.locations << Location.create(name: "Guatemala", slug: 'guatemala', :location_type => :country)
o.locations << Location.create(name: "Haiti", slug: 'haiti', :location_type => :country)
o.locations << Location.create(name: "Honduras", slug: 'honduras', :location_type => :country)
o.locations << Location.create(name: "Jamaica", slug: 'jamaica', :location_type => :country)
o.locations << Location.create(name: "Mexico", slug: 'mexico', :location_type => :country)
o.locations << Location.create(name: "Nicaragua", slug: 'nicaragua', :location_type => :country)
o.locations << Location.create(name: "Egypt", slug: 'egypt', :location_type => :country)
o.locations << Location.create(name: "Jordan", slug: 'jordan', :location_type => :country)
o.locations << Location.create(name: "United States", slug: 'united_states', :location_type => :country)
o.locations << Location.create(name: "Afghanistan", slug: 'afghanistan', :location_type => :country)
o.locations << Location.create(name: "Bangladesh", slug: 'bangladesh', :location_type => :country)
o.locations << Location.create(name: "India", slug: 'india', :location_type => :country)
o.locations << Location.create(name: "Kazakhstan", slug: 'kazakhstan', :location_type => :country)
o.locations << Location.create(name: "Kyrgystan", slug: 'kyrgystan', :location_type => :country)
o.locations << Location.create(name: "Nepal", slug: 'nepal', :location_type => :country)
o.locations << Location.create(name: "Tajikistan", slug: 'tajikistan', :location_type => :country)
o.locations << Location.create(name: "Turkmenistan", slug: 'turkmenistan', :location_type => :country)
o.locations << Location.create(name: "Ukraine", slug: 'ukraine', :location_type => :country)
o.locations << Location.create(name: "Uzbekistan", slug: 'uzbekistan', :location_type => :country)
o.locations << Location.create(name: "Angola", slug: 'angola', :location_type => :country)
o.locations << Location.create(name: "Burkina Faso", slug: 'burkina_faso', :location_type => :country)
o.locations << Location.create(name: "Benin", slug: 'benin', :location_type => :country)
o.locations << Location.create(name: "Burundi", slug: 'burundi', :location_type => :country)
o.locations << Location.create(name: "Botswana", slug: 'botswana', :location_type => :country)
o.locations << Location.create(name: "Cote d’Ivoire", slug: 'cote_divoire', :location_type => :country)
o.locations << Location.create(name: "Democratic Republic of the Congo", slug: 'democratic_republic_of_the_congo', :location_type => :country)
o.locations << Location.create(name: "Ethiopia", slug: 'ethiopia', :location_type => :country)
o.locations << Location.create(name: "Ghana", slug: 'ghana', :location_type => :country)
o.locations << Location.create(name: "Guinea", slug: 'guinea', :location_type => :country)
o.locations << Location.create(name: "Kenya", slug: 'kenya', :location_type => :country)
o.locations << Location.create(name: "Lesotho", slug: 'lesotho', :location_type => :country)
o.locations << Location.create(name: "Liberia", slug: 'liberia', :location_type => :country)
o.locations << Location.create(name: "Madagascar", slug: 'madagascar', :location_type => :country)
o.locations << Location.create(name: "Mali", slug: 'mali', :location_type => :country)
o.locations << Location.create(name: "Mozambique", slug: 'mozambique', :location_type => :country)
o.locations << Location.create(name: "Namibia", slug: 'namibia', :location_type => :country)
o.locations << Location.create(name: "Niger", slug: 'niger', :location_type => :country)
o.locations << Location.create(name: "Nigeria", slug: 'nigeria', :location_type => :country)
o.locations << Location.create(name: "Rwanda", slug: 'rwanda', :location_type => :country)
o.locations << Location.create(name: "Senegal", slug: 'senegal', :location_type => :country)
o.locations << Location.create(name: "Sierra Leone", slug: 'sierra_leone', :location_type => :country)
o.locations << Location.create(name: "South Sudan", slug: 'south_sudan', :location_type => :country)
o.locations << Location.create(name: "Tanzania", slug: 'tanzania', :location_type => :country)
o.locations << Location.create(name: "Uganda", slug: 'uganda', :location_type => :country)
o.locations << Location.create(name: "Zambia", slug: 'zambia', :location_type => :country)
o.locations << Location.create(name: "Zimbabwe", slug: 'zimbabwe', :location_type => :country)
o = Organization.create(name: "Aerian", slug: "aerian", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.aerian.com")

o.sectors << Sector.where(slug: 'ictd').limit(1)[0]
o.sectors << Sector.where(slug: 'platform_creation').limit(1)[0]
l = Location.create(name: "The Old Malthouse, Mill Lane, UK", slug: 'the_old_malthouse_mill_lane_uk', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (51.41965159999999, -2.253189) ] WHERE slug = 'the_old_malthouse_mill_lane_uk'")
o.locations << l
o.locations << Location.where(slug: 'united_kingdom').limit(1)[0]
o = Organization.create(name: "Akros", slug: "akros", when_endorsed: DateTime.new(2017, 1, 1), is_endorser: true, website: "www.akros.com")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'information_systems').limit(1)[0]
o.sectors << Sector.where(slug: 'agriculture').limit(1)[0]
o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.create(name: "Malawi", slug: 'malawi', :location_type => :country)
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o.locations << Location.create(name: "Swaziland", slug: 'swaziland', :location_type => :country)
o.locations << Location.create(name: "South Africa", slug: 'south_africa', :location_type => :country)
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.where(slug: 'namibia').limit(1)[0]
o = Organization.create(name: "Akryl", slug: "akryl", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.akryl.net ")

o.sectors << Sector.where(slug: 'communication').limit(1)[0]
l = Location.create(name: "Beijing, China", slug: 'beijing_china', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (39.90419989999999, 116.4073963) ] WHERE slug = 'beijing_china'")
o.locations << l
l = Location.create(name: "Hamburg, Germany", slug: 'hamburg_germany', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (53.5510846, 9.9936819) ] WHERE slug = 'hamburg_germany'")
o.locations << l
o.locations << Location.create(name: "China", slug: 'china', :location_type => :country)
o.locations << Location.create(name: "Germany", slug: 'germany', :location_type => :country)
o.locations << Location.create(name: "(might have other clients in other European and Asian countries", slug: 'might_have_other_clients_in_othe', :location_type => :country)
o.locations << Location.create(name: "based on what their website says)", slug: 'based_on_what_their_website_says', :location_type => :country)
o = Organization.create(name: "Aptivate", slug: "aptivate", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "http://www.aptivate.org/")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'gender').limit(1)[0]
o.locations << Location.where(slug: 'united_kingdom').limit(1)[0]
o = Organization.create(name: "Avallain Foundation, inc.", slug: "avallain_foundation_inc", when_endorsed: DateTime.new(2017, 1, 1), is_endorser: true, website: "www.avallainfoundation.org")

o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'agriculture').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o = Organization.create(name: "Awaaz.De", slug: "awaazde", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://www.awaaz.de/")

o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'finance').limit(1)[0]
o.sectors << Sector.where(slug: 'agriculture').limit(1)[0]
l = Location.create(name: "Ahmedabad, Gujrat, India", slug: 'ahmedabad_gujrat_india', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (23.022505, 72.5713621) ] WHERE slug = 'ahmedabad_gujrat_india'")
o.locations << l
o.locations << Location.where(slug: 'india').limit(1)[0]
o = Organization.create(name: "Belgian Development Agency", slug: "belgian_development_agency", when_endorsed: DateTime.new(2017, 1, 1), is_endorser: true, website: "WWW.BTCCTB.ORG")

o.sectors << Sector.where(slug: 'agriculture').limit(1)[0]
o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'employment').limit(1)[0]
o.sectors << Sector.where(slug: 'energy').limit(1)[0]
o.sectors << Sector.where(slug: 'environemnt').limit(1)[0]
o.sectors << Sector.where(slug: 'gender').limit(1)[0]
o.sectors << Sector.where(slug: 'governance').limit(1)[0]
o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'water').limit(1)[0]
l = Location.create(name: "Brussels, Belgium", slug: 'brussels_belgium', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (50.8503463, 4.3517211) ] WHERE slug = 'brussels_belgium'")
o.locations << l
o.locations << Location.create(name: "Algeria", slug: 'algeria', :location_type => :country)
o.locations << Location.create(name: "Belgium", slug: 'belgium', :location_type => :country)
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.create(name: "Bolivia", slug: 'bolivia', :location_type => :country)
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.create(name: "DR Congo", slug: 'dr_congo', :location_type => :country)
o.locations << Location.create(name: "Ecuador", slug: 'ecuador', :location_type => :country)
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.create(name: "Morocco", slug: 'morocco', :location_type => :country)
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.create(name: "Palestiniam Territory", slug: 'palestiniam_territory', :location_type => :country)
o.locations << Location.create(name: "Peru", slug: 'peru', :location_type => :country)
o.locations << Location.where(slug: 'rwanda').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o = Organization.create(name: "Bill and Melinda Gates Foundation", slug: "bill_and_melinda_gates_foundatio", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.gatesfoundation.org/")

o.sectors << Sector.where(slug: 'agriculture').limit(1)[0]
o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'employment').limit(1)[0]
o.sectors << Sector.where(slug: 'energy').limit(1)[0]
o.sectors << Sector.where(slug: 'environemnt').limit(1)[0]
o.sectors << Sector.where(slug: 'gender').limit(1)[0]
o.sectors << Sector.where(slug: 'governance').limit(1)[0]
o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'water').limit(1)[0]
l = Location.create(name: "Seattle, WA", slug: 'seattle_wa', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (47.6062095, -122.3320708) ] WHERE slug = 'seattle_wa'")
o.locations << l
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'united_kingdom').limit(1)[0]
o.locations << Location.create(name: "France", slug: 'france', :location_type => :country)
o.locations << Location.where(slug: 'germany').limit(1)[0]
o.locations << Location.create(name: "Norway", slug: 'norway', :location_type => :country)
o.locations << Location.create(name: "Sweden", slug: 'sweden', :location_type => :country)
o.locations << Location.create(name: "Netherlands", slug: 'netherlands', :location_type => :country)
o.locations << Location.create(name: "(Middle East: Partnerships)", slug: 'middle_east_partnerships', :location_type => :country)
o = Organization.create(name: "Bivee", slug: "bivee", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.bivee.co")

o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o.locations << Location.create(name: "Hong Kong", slug: 'hong_kong', :location_type => :country)
o = Organization.create(name: "Build Up", slug: "build_up", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://howtobuildup.org/projects/ ")

o = Organization.create(name: "Caktus Group", slug: "caktus_group", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://www.caktusgroup.com/")

o.sectors << Sector.where(slug: 'energy').limit(1)[0]
o.sectors << Sector.where(slug: 'finance_and_insurance').limit(1)[0]
o.sectors << Sector.where(slug: 'media').limit(1)[0]
o.sectors << Sector.where(slug: 'entertainment').limit(1)[0]
o.sectors << Sector.where(slug: 'travel_and_hospitality').limit(1)[0]
o.sectors << Sector.where(slug: 'membership_associations').limit(1)[0]
o.sectors << Sector.where(slug: 'higher_education').limit(1)[0]
o.sectors << Sector.where(slug: 'research_collaborations').limit(1)[0]
o.sectors << Sector.where(slug: 'engineering').limit(1)[0]
o.sectors << Sector.where(slug: 'ictd').limit(1)[0]
o.sectors << Sector.where(slug: 'ngo').limit(1)[0]
o.sectors << Sector.where(slug: 'government').limit(1)[0]
l = Location.create(name: "Durham, NC", slug: 'durham_nc', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (35.9940329, -78.898619) ] WHERE slug = 'durham_nc'")
o.locations << l
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o = Organization.create(name: "Catholic Relief Services (CRS)", slug: "catholic_relief_services_crs", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.crs.org/")

o.sectors << Sector.where(slug: 'agriculture').limit(1)[0]
o.sectors << Sector.where(slug: 'emergency').limit(1)[0]
o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'microfinance').limit(1)[0]
o.sectors << Sector.where(slug: 'water').limit(1)[0]
o.sectors << Sector.where(slug: 'justice__peacebuilding').limit(1)[0]
o.sectors << Sector.where(slug: 'youth').limit(1)[0]
l = Location.create(name: "Baltimore, MD", slug: 'baltimore_md', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (39.2903848, -76.6121893) ] WHERE slug = 'baltimore_md'")
o.locations << l
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.create(name: "Burundu", slug: 'burundu', :location_type => :country)
o.locations << Location.create(name: "Cameroon", slug: 'cameroon', :location_type => :country)
o.locations << Location.create(name: "Central African Republic", slug: 'central_african_republic', :location_type => :country)
o.locations << Location.create(name: "Chad", slug: 'chad', :location_type => :country)
o.locations << Location.create(name: "Cote d'lvoire", slug: 'cote_dlvoire', :location_type => :country)
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.create(name: "Equatorial Guinea", slug: 'equatorial_guinea', :location_type => :country)
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.create(name: "Guinea-Bissau", slug: 'guineabissau', :location_type => :country)
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'lesotho').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.create(name: "Mauritania", slug: 'mauritania', :location_type => :country)
o.locations << Location.where(slug: 'namibia').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.create(name: "Republic of the Congo", slug: 'republic_of_the_congo', :location_type => :country)
o.locations << Location.create(name: "Rwana", slug: 'rwana', :location_type => :country)
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.create(name: "Somalia", slug: 'somalia', :location_type => :country)
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.create(name: "Sudan", slug: 'sudan', :location_type => :country)
o.locations << Location.where(slug: 'swaziland').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.create(name: "Tha Gambia", slug: 'tha_gambia', :location_type => :country)
o.locations << Location.create(name: "Togo", slug: 'togo', :location_type => :country)
o.locations << Location.create(name: "Tunisia", slug: 'tunisia', :location_type => :country)
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.create(name: "Armenia", slug: 'armenia', :location_type => :country)
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'laos').limit(1)[0]
o.locations << Location.create(name: "Micronesia", slug: 'micronesia', :location_type => :country)
o.locations << Location.create(name: "Myanmar", slug: 'myanmar', :location_type => :country)
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.create(name: "Pakistan", slug: 'pakistan', :location_type => :country)
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.create(name: "Russia", slug: 'russia', :location_type => :country)
o.locations << Location.create(name: "Sri Lanka", slug: 'sri_lanka', :location_type => :country)
o.locations << Location.create(name: "Tajikstan", slug: 'tajikstan', :location_type => :country)
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.create(name: "The Pacific Sub- Region", slug: 'the_pacific_sub_region', :location_type => :country)
o.locations << Location.create(name: "Timor-Leste (East Timor)", slug: 'timorleste_east_timor', :location_type => :country)
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o.locations << Location.where(slug: 'bolivia').limit(1)[0]
o.locations << Location.create(name: "Brazil", slug: 'brazil', :location_type => :country)
o.locations << Location.create(name: "Colombia", slug: 'colombia', :location_type => :country)
o.locations << Location.create(name: "Chile", slug: 'chile', :location_type => :country)
o.locations << Location.create(name: "Cuba", slug: 'cuba', :location_type => :country)
o.locations << Location.where(slug: 'dominican_republic').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.create(name: "Guyana", slug: 'guyana', :location_type => :country)
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'jamaica').limit(1)[0]
o.locations << Location.create(name: "Lesser Antilles", slug: 'lesser_antilles', :location_type => :country)
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.create(name: "Greece", slug: 'greece', :location_type => :country)
o.locations << Location.create(name: "Albania", slug: 'albania', :location_type => :country)
o.locations << Location.create(name: "Bosnia and Herzegovina", slug: 'bosnia_and_herzegovina', :location_type => :country)
o.locations << Location.create(name: "Kosovo", slug: 'kosovo', :location_type => :country)
o.locations << Location.create(name: "Serbia", slug: 'serbia', :location_type => :country)
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.create(name: "Iraq", slug: 'iraq', :location_type => :country)
o.locations << Location.create(name: "Jerusalem", slug: 'jerusalem', :location_type => :country)
o.locations << Location.create(name: "West Bank and Gaza", slug: 'west_bank_and_gaza', :location_type => :country)
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.create(name: "Lebanon", slug: 'lebanon', :location_type => :country)
o = Organization.create(name: "Center for International Private Enterprise (CIPE)", slug: "center_for_international_private", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.cipe.org/")

o.sectors << Sector.where(slug: 'anticorruption__ethics').limit(1)[0]
o.sectors << Sector.where(slug: 'enterprise_ecosystems').limit(1)[0]
o.sectors << Sector.where(slug: 'business_advocacy').limit(1)[0]
o.sectors << Sector.where(slug: 'democratic_governance').limit(1)[0]
o.sectors << Sector.where(slug: 'trade').limit(1)[0]
o.sectors << Sector.where(slug: 'centers_of_excellence').limit(1)[0]
l = Location.create(name: "Washington, DC", slug: 'washington_dc', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (38.9071923, -77.0368707) ] WHERE slug = 'washington_dc'")
o.locations << l
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'papua_new_guinea').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.create(name: "Cambodias", slug: 'cambodias', :location_type => :country)
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.create(name: "Moldova", slug: 'moldova', :location_type => :country)
o.locations << Location.where(slug: 'kyrgystan').limit(1)[0]
o.locations << Location.create(name: "Georgia", slug: 'georgia', :location_type => :country)
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.create(name: "Ukraine Colombia", slug: 'ukraine_colombia', :location_type => :country)
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.create(name: "Paraguay", slug: 'paraguay', :location_type => :country)
o.locations << Location.create(name: "Argentina", slug: 'argentina', :location_type => :country)
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.create(name: "Bahrain", slug: 'bahrain', :location_type => :country)
o.locations << Location.create(name: "Yemen", slug: 'yemen', :location_type => :country)
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'algeria').limit(1)[0]
o.locations << Location.create(name: "Syria", slug: 'syria', :location_type => :country)
o.locations << Location.create(name: "Palestinian Territories", slug: 'palestinian_territories', :location_type => :country)
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o = Organization.create(name: "Chamilo", slug: "chamilo", when_endorsed: DateTime.new(2017, 1, 1), is_endorser: true, website: "www.chamilo.org")

o = Organization.create(name: "Charlie Goldsmith Associates", slug: "charlie_goldsmith_associates", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://charliegoldsmithassociates.co.uk/")

o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'food').limit(1)[0]
o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'income__local_markets').limit(1)[0]
o.sectors << Sector.where(slug: 'financial').limit(1)[0]
o.sectors << Sector.where(slug: 'management').limit(1)[0]
o.sectors << Sector.where(slug: 'protection').limit(1)[0]
l = Location.create(name: "London, UK", slug: 'london_uk', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (51.5073509, -0.1277583) ] WHERE slug = 'london_uk'")
o.locations << l
o.locations << Location.where(slug: 'united_kingdom').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.create(name: "Rawanda", slug: 'rawanda', :location_type => :country)
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o = Organization.create(name: "Chemonics", slug: "chemonics", when_endorsed: DateTime.new(2017, 1, 1), is_endorser: true, website: "www.chemonics.com")

o.sectors << Sector.where(slug: 'agriculture_and_food_security').limit(1)[0]
o.sectors << Sector.where(slug: 'corporate_partnerships').limit(1)[0]
o.sectors << Sector.where(slug: 'democracy_and_governance').limit(1)[0]
o.sectors << Sector.where(slug: 'economic_growth_and_trade').limit(1)[0]
o.sectors << Sector.where(slug: 'education_and_youth').limit(1)[0]
o.sectors << Sector.where(slug: 'environment_and_natural_resource').limit(1)[0]
o.sectors << Sector.where(slug: 'gender_equality_and_social_inclu').limit(1)[0]
o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'peace').limit(1)[0]
o.sectors << Sector.where(slug: 'stability').limit(1)[0]
o.sectors << Sector.where(slug: 'and_transition').limit(1)[0]
o.sectors << Sector.where(slug: 'supply_chain_solutions').limit(1)[0]
o.sectors << Sector.where(slug: 'water').limit(1)[0]
o.sectors << Sector.where(slug: 'energy').limit(1)[0]
o.sectors << Sector.where(slug: 'and_sustainable_cities').limit(1)[0]
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.create(name: "Maldives", slug: 'maldives', :location_type => :country)
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'tajikstan').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'palestinian_territories').limit(1)[0]
o = Organization.create(name: "CODE Innovation", slug: "code_innovation", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://codeinnovation.com/")

o.sectors << Sector.where(slug: 'governance').limit(1)[0]
o.sectors << Sector.where(slug: 'finance').limit(1)[0]
o.sectors << Sector.where(slug: 'education').limit(1)[0]
l = Location.create(name: "Silicon Valley?", slug: 'silicon_valley', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (37.387474, -122.0575434) ] WHERE slug = 'silicon_valley'")
o.locations << l
o.locations << Location.create(name: "Palestine", slug: 'palestine', :location_type => :country)
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o = Organization.create(name: "Congo in the Picture", slug: "congo_in_the_picture", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "https://www.facebook.com/CongoInThePicture/")

o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o = Organization.create(name: "Cooper/Smith", slug: "coopersmith", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "coopersmith.org")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o = Organization.create(name: "Creative Associates International", slug: "creative_associates_internationa", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.creativeassociatesinternational.com/")

o.sectors << Sector.where(slug: 'capacity_development').limit(1)[0]
o.sectors << Sector.where(slug: 'citizen_security').limit(1)[0]
o.sectors << Sector.where(slug: 'cve_and_pve').limit(1)[0]
o.sectors << Sector.where(slug: 'digital_development').limit(1)[0]
o.sectors << Sector.where(slug: 'education_in_conflict').limit(1)[0]
o.sectors << Sector.where(slug: 'elections').limit(1)[0]
o.sectors << Sector.where(slug: 'gender').limit(1)[0]
o.sectors << Sector.where(slug: 'governance').limit(1)[0]
o.sectors << Sector.where(slug: 'instructional_systems').limit(1)[0]
o.sectors << Sector.where(slug: 'livelihoods').limit(1)[0]
o.sectors << Sector.where(slug: 'political_transitions').limit(1)[0]
o.sectors << Sector.where(slug: 'stabilization').limit(1)[0]
o.sectors << Sector.where(slug: 'workforce').limit(1)[0]
o.sectors << Sector.where(slug: 'youth').limit(1)[0]
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'turkmenistan').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.create(name: "Libya", slug: 'libya', :location_type => :country)
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'guyana').limit(1)[0]
o.locations << Location.create(name: "Saint Lucia", slug: 'saint_lucia', :location_type => :country)
o.locations << Location.create(name: "Saint Kitts and Nevis", slug: 'saint_kitts_and_nevis', :location_type => :country)
o.locations << Location.create(name: "El Salavador", slug: 'el_salavador', :location_type => :country)
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.create(name: "Other countries:", slug: 'other_countries', :location_type => :country)
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.create(name: "Bolovia", slug: 'bolovia', :location_type => :country)
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.where(slug: 'dominican_republic').limit(1)[0]
o.locations << Location.where(slug: 'jamaica').limit(1)[0]
o.locations << Location.create(name: "Belize", slug: 'belize', :location_type => :country)
o.locations << Location.create(name: "Costa Rica", slug: 'costa_rica', :location_type => :country)
o.locations << Location.create(name: "Panama", slug: 'panama', :location_type => :country)
o.locations << Location.where(slug: 'algeria').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.create(name: "S. Sudan", slug: 's_sudan', :location_type => :country)
o.locations << Location.create(name: "Sengal", slug: 'sengal', :location_type => :country)
o.locations << Location.create(name: "Gambia", slug: 'gambia', :location_type => :country)
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.create(name: "Ivory Coast", slug: 'ivory_coast', :location_type => :country)
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.create(name: "Eq. Guinea", slug: 'eq_guinea', :location_type => :country)
o.locations << Location.create(name: "Congo", slug: 'congo', :location_type => :country)
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.create(name: "Nambia", slug: 'nambia', :location_type => :country)
o.locations << Location.where(slug: 'swaziland').limit(1)[0]
o.locations << Location.where(slug: 'lesotho').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.create(name: "Yemem", slug: 'yemem', :location_type => :country)
o.locations << Location.create(name: "Oman", slug: 'oman', :location_type => :country)
o.locations << Location.where(slug: 'palestine').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.create(name: "Azerbaijan", slug: 'azerbaijan', :location_type => :country)
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'russia').limit(1)[0]
o.locations << Location.where(slug: 'kazakhstan').limit(1)[0]
o.locations << Location.where(slug: 'uzbekistan').limit(1)[0]
o.locations << Location.where(slug: 'turkmenistan').limit(1)[0]
o.locations << Location.where(slug: 'kyrgystan').limit(1)[0]
o.locations << Location.where(slug: 'tajikstan').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.create(name: "Bhutan", slug: 'bhutan', :location_type => :country)
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.create(name: "Belarus", slug: 'belarus', :location_type => :country)
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'moldova').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.create(name: "Romania", slug: 'romania', :location_type => :country)
o.locations << Location.create(name: "Bulgaria", slug: 'bulgaria', :location_type => :country)
o.locations << Location.where(slug: 'albania').limit(1)[0]
o.locations << Location.where(slug: 'serbia').limit(1)[0]
o.locations << Location.create(name: "Bosnia and Herz.", slug: 'bosnia_and_herz', :location_type => :country)
o.locations << Location.create(name: "Slovenia", slug: 'slovenia', :location_type => :country)
o.locations << Location.create(name: "Portugal", slug: 'portugal', :location_type => :country)
o = Organization.create(name: "Democracy International", slug: "democracy_international", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://democracyinternational.com/")

o.sectors << Sector.where(slug: 'governance').limit(1)[0]
l = Location.create(name: "Bethesda, MD", slug: 'bethesda_md', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (38.984652, -77.0947092) ] WHERE slug = 'bethesda_md'")
o.locations << l
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.create(name: "Gautemala", slug: 'gautemala', :location_type => :country)
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'panama').limit(1)[0]
o.locations << Location.where(slug: 'jamaica').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.create(name: "Venezuela", slug: 'venezuela', :location_type => :country)
o.locations << Location.where(slug: 'guyana').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.where(slug: 'bolivia').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'sengal').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'ivory_coast').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.create(name: "Tunsia", slug: 'tunsia', :location_type => :country)
o.locations << Location.where(slug: 'libya').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 's_sudan').limit(1)[0]
o.locations << Location.where(slug: 'central_african_republic').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.create(name: "Kyrgyzstan", slug: 'kyrgyzstan', :location_type => :country)
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.create(name: "Malaysia", slug: 'malaysia', :location_type => :country)
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'russia').limit(1)[0]
o.locations << Location.where(slug: 'azerbaijan').limit(1)[0]
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'moldova').limit(1)[0]
o.locations << Location.where(slug: 'romania').limit(1)[0]
o.locations << Location.where(slug: 'serbia').limit(1)[0]
o.locations << Location.create(name: "Bosnia & Herz.", slug: 'bosnia__herz', :location_type => :country)
o.locations << Location.create(name: "Montenegro", slug: 'montenegro', :location_type => :country)
o.locations << Location.where(slug: 'albania').limit(1)[0]
o.locations << Location.where(slug: 'kosovo').limit(1)[0]
o.locations << Location.create(name: "Macedonia", slug: 'macedonia', :location_type => :country)
o = Organization.create(name: "Development Alternatives Incorporated (DAI)", slug: "development_alternatives_incorpo", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://www.dai.com/")

o.sectors << Sector.where(slug: 'digital_acceleration').limit(1)[0]
o.sectors << Sector.where(slug: 'economic_growth').limit(1)[0]
o.sectors << Sector.where(slug: 'environment__energy').limit(1)[0]
o.sectors << Sector.where(slug: 'fragile_states').limit(1)[0]
o.sectors << Sector.where(slug: 'global_health').limit(1)[0]
o.sectors << Sector.where(slug: 'governance').limit(1)[0]
o.sectors << Sector.where(slug: 'sustainable_business').limit(1)[0]
o.locations << Location.where(slug: 'bethesda_md').limit(1)[0]
o.locations << Location.create(name: "Offices: Belgium", slug: 'offices_belgium', :location_type => :country)
o.locations << Location.where(slug: 'united_kingdom').limit(1)[0]
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o.locations << Location.create(name: "countries w/ active projects: Poland", slug: 'countries_w_active_projects_pola', :location_type => :country)
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'moldova').limit(1)[0]
o.locations << Location.where(slug: 'bosnia__herz').limit(1)[0]
o.locations << Location.where(slug: 'kosovo').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'palestine').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'gambia').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'ivory_coast').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.create(name: "Congo (Kinshasa)", slug: 'congo_kinshasa', :location_type => :country)
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.create(name: "Smoliland", slug: 'smoliland', :location_type => :country)
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.where(slug: 'namibia').limit(1)[0]
o.locations << Location.where(slug: 'swaziland').limit(1)[0]
o.locations << Location.where(slug: 'lesotho').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.create(name: "Seychelles", slug: 'seychelles', :location_type => :country)
o.locations << Location.where(slug: 'kazakhstan').limit(1)[0]
o.locations << Location.where(slug: 'kyrgyzstan').limit(1)[0]
o.locations << Location.where(slug: 'uzbekistan').limit(1)[0]
o.locations << Location.where(slug: 'turkmenistan').limit(1)[0]
o.locations << Location.where(slug: 'tajikstan').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'laos').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o.locations << Location.create(name: "Singapore", slug: 'singapore', :location_type => :country)
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'malaysia').limit(1)[0]
o.locations << Location.create(name: "Brunei", slug: 'brunei', :location_type => :country)
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.create(name: "Guam", slug: 'guam', :location_type => :country)
o.locations << Location.create(name: "Fiji", slug: 'fiji', :location_type => :country)
o.locations << Location.create(name: "American Samoa", slug: 'american_samoa', :location_type => :country)
o = Organization.create(name: "Development Gateway", slug: "development_gateway", when_endorsed: DateTime.new(2017, 1, 1), is_endorser: true, website: "www.developmentgateway.org")

o.sectors << Sector.where(slug: 'data_management__policy').limit(1)[0]
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.create(name: "Malwai", slug: 'malwai', :location_type => :country)
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'chad').limit(1)[0]
o.locations << Location.where(slug: 'ivory_coast').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.create(name: "The Gambia", slug: 'the_gambia', :location_type => :country)
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.create(name: "Hondura", slug: 'hondura', :location_type => :country)
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o.locations << Location.create(name: "Canada", slug: 'canada', :location_type => :country)
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'chile').limit(1)[0]
o.locations << Location.where(slug: 'kosovo').limit(1)[0]
o.locations << Location.create(name: "Maldova", slug: 'maldova', :location_type => :country)
o.locations << Location.where(slug: 'norway').limit(1)[0]
o.locations << Location.where(slug: 'united_kingdom').limit(1)[0]
o.locations << Location.where(slug: 'timorleste').limit(1)[0]
o.locations << Location.create(name: "Lao", slug: 'lao', :location_type => :country)
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'kyrgyzstan').limit(1)[0]
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.create(name: "Mongolia", slug: 'mongolia', :location_type => :country)
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'papua_new_guinea').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o = Organization.create(name: "Digital Campus", slug: "digital_campus", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://digital-campus.org/")

o.sectors << Sector.where(slug: 'health__consultancy').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o = Organization.create(name: "Digital Green", slug: "digital_green", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.digitalgreen.org/")

o.sectors << Sector.where(slug: 'agriculturepoverty').limit(1)[0]
l = Location.create(name: "San Francisco, CA", slug: 'san_francisco_ca', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (37.7749295, -122.4194155) ] WHERE slug = 'san_francisco_ca'")
o.locations << l
l = Location.create(name: "New Delhi, India", slug: 'new_delhi_india', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (28.6139391, 77.2090212) ] WHERE slug = 'new_delhi_india'")
o.locations << l
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o = Organization.create(name: "Digital Impact Alliance (DIAL)", slug: "digital_impact_alliance_dial", when_endorsed: DateTime.new(2017, 1, 1), is_endorser: true, website: "http://digitalimpactalliance.org/")

o = Organization.create(name: "Digital Opportunity Trust (DOT)", slug: "digital_opportunity_trust_dot", when_endorsed: DateTime.new(2017, 1, 1), is_endorser: true, website: "https://www.dotrust.org/")

o.sectors << Sector.where(slug: 'youth').limit(1)[0]
o.sectors << Sector.where(slug: 'education').limit(1)[0]
l = Location.create(name: "Ottawa, Canada", slug: 'ottawa_canada', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (45.4215296, -75.69719309999999) ] WHERE slug = 'ottawa_canada'")
o.locations << l
o.locations << Location.where(slug: 'canada').limit(1)[0]
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'panama').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'chile').limit(1)[0]
o.locations << Location.where(slug: 'argentina').limit(1)[0]
o.locations << Location.where(slug: 'united_kingdom').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.create(name: "United Arab Emirates", slug: 'united_arab_emirates', :location_type => :country)
o.locations << Location.create(name: "Turkey", slug: 'turkey', :location_type => :country)
o.locations << Location.where(slug: 'china').limit(1)[0]
o = Organization.create(name: "Dimagi", slug: "dimagi", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.dimagi.com")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'communications').limit(1)[0]
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o = Organization.create(name: "Disruptive Designs", slug: "disruptive_designs", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "")

o = Organization.create(name: "Ecorys", slug: "ecorys", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "https://uk.ecorys.com/")

o = Organization.create(name: "Education Development Center (EDC)", slug: "education_development_center_edc", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.edc.org/")

o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'health').limit(1)[0]
l = Location.create(name: "Waltham (Boston), MA", slug: 'waltham_boston_ma', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (42.3764852, -71.2356113) ] WHERE slug = 'waltham_boston_ma'")
o.locations << l
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o.locations << Location.create(name: "Africa", slug: 'africa', :location_type => :country)
o.locations << Location.create(name: "Europe", slug: 'europe', :location_type => :country)
o.locations << Location.create(name: "Middle East", slug: 'middle_east', :location_type => :country)
o.locations << Location.create(name: "Latin America", slug: 'latin_america', :location_type => :country)
o.locations << Location.create(name: "(Countries are not listed)", slug: 'countries_are_not_listed', :location_type => :country)
o = Organization.create(name: "Enabel", slug: "enabel", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.enabel.be")

o.sectors << Sector.where(slug: 'agriculture__rural_development').limit(1)[0]
o.sectors << Sector.where(slug: 'digitalization').limit(1)[0]
o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'training__employment').limit(1)[0]
o.sectors << Sector.where(slug: 'energy').limit(1)[0]
o.sectors << Sector.where(slug: 'environment__climate').limit(1)[0]
o.sectors << Sector.where(slug: 'gender').limit(1)[0]
o.sectors << Sector.where(slug: 'governance').limit(1)[0]
o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'water__sanitation').limit(1)[0]
o.locations << Location.where(slug: 'brussels_belgium').limit(1)[0]
o.locations << Location.where(slug: 'algeria').limit(1)[0]
o.locations << Location.where(slug: 'belgium').limit(1)[0]
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'bolivia').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'palestine').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o = Organization.create(name: "Every1Mobile", slug: "everymobile", when_endorsed: DateTime.new(2015, 1, 1), is_endorser: true, website: "http://www.every1mobile.net/")

o.locations << Location.where(slug: 'united_kingdom').limit(1)[0]
o = Organization.create(name: "FAIR Denmark", slug: "fair_denmark", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "https://www.fairdanmark.dk/en/")

l = Location.create(name: "Copenhagen, Denmark", slug: 'copenhagen_denmark', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (55.6760968, 12.5683372) ] WHERE slug = 'copenhagen_denmark'")
o.locations << l
o.locations << Location.create(name: "Norway and Denmark", slug: 'norway_and_denmark', :location_type => :country)
o = Organization.create(name: "FHI 360", slug: "fhi_", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://www.fhi360.org/")

o.sectors << Sector.where(slug: 'civil_society').limit(1)[0]
o.sectors << Sector.where(slug: 'communication_and_social_marketi').limit(1)[0]
o.sectors << Sector.where(slug: 'crisis_response').limit(1)[0]
o.sectors << Sector.where(slug: 'economic_development').limit(1)[0]
o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'environment').limit(1)[0]
o.sectors << Sector.where(slug: 'gender').limit(1)[0]
o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'nutrition').limit(1)[0]
o.sectors << Sector.where(slug: 'research').limit(1)[0]
o.sectors << Sector.where(slug: 'technology').limit(1)[0]
o.sectors << Sector.where(slug: 'youth').limit(1)[0]
o.locations << Location.where(slug: 'durham_nc').limit(1)[0]
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'jamaica').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'ivory_coast').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'eq_guinea').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.where(slug: 'namibia').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'swaziland').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'laos').limit(1)[0]
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o = Organization.create(name: "Fio", slug: "fio", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.fio.com")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
l = Location.create(name: "Toronto, ON, Canada", slug: 'toronto_on_canada', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (43.653226, -79.3831843) ] WHERE slug = 'toronto_on_canada'")
o.locations << l
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.create(name: "DRC", slug: 'drc', :location_type => :country)
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o = Organization.create(name: "Forum One", slug: "forum_one", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://forumone.com/")

o.sectors << Sector.where(slug: 'humanitarian__poverty').limit(1)[0]
o.sectors << Sector.where(slug: 'hunger').limit(1)[0]
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o = Organization.create(name: "Foundation Botnar", slug: "foundation_botnar", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "https://www.fondationbotnar.org/en")

o.sectors << Sector.where(slug: 'youth').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'romania').limit(1)[0]
o.locations << Location.create(name: "Switzerland", slug: 'switzerland', :location_type => :country)
o.locations << Location.create(name: "Israel", slug: 'israel', :location_type => :country)
o = Organization.create(name: "Girl Effect Mobile - Springster ", slug: "girl_effect_mobile__springster", when_endorsed: DateTime.new(2017, 1, 1), is_endorser: true, website: "girleffect.org")

o.sectors << Sector.where(slug: 'youth').limit(1)[0]
o.sectors << Sector.where(slug: 'gender').limit(1)[0]
o.locations << Location.where(slug: 'london_uk').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o = Organization.create(name: "GIZ", slug: "giz", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "https://www.giz.de/en/html/index.html")

o.sectors << Sector.where(slug: 'governance').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'algeria').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'ivory_coast').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'lesotho').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'mauritania').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'namibia').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 's_sudan').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'togo').limit(1)[0]
o.locations << Location.where(slug: 'chad').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'central_african_republic').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'kazakhstan').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'kyrgyzstan').limit(1)[0]
o.locations << Location.where(slug: 'laos').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'malaysia').limit(1)[0]
o.locations << Location.where(slug: 'maldives').limit(1)[0]
o.locations << Location.where(slug: 'mongolia').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'palestine').limit(1)[0]
o.locations << Location.create(name: "Oceania", slug: 'oceania', :location_type => :country)
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.create(name: "Saudi Arabia", slug: 'saudi_arabia', :location_type => :country)
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'syria').limit(1)[0]
o.locations << Location.where(slug: 'tajikistan').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'timorleste').limit(1)[0]
o.locations << Location.where(slug: 'uzbekistan').limit(1)[0]
o.locations << Location.where(slug: 'united_arab_emirates').limit(1)[0]
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o.locations << Location.where(slug: 'albania').limit(1)[0]
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.where(slug: 'azerbaijan').limit(1)[0]
o.locations << Location.where(slug: 'belarus').limit(1)[0]
o.locations << Location.where(slug: 'bosnia_and_herz').limit(1)[0]
o.locations << Location.where(slug: 'bulgaria').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'greece').limit(1)[0]
o.locations << Location.where(slug: 'kosovo').limit(1)[0]
o.locations << Location.where(slug: 'macedonia').limit(1)[0]
o.locations << Location.where(slug: 'moldova').limit(1)[0]
o.locations << Location.where(slug: 'montenegro').limit(1)[0]
o.locations << Location.where(slug: 'romania').limit(1)[0]
o.locations << Location.where(slug: 'russia').limit(1)[0]
o.locations << Location.where(slug: 'serbia').limit(1)[0]
o.locations << Location.where(slug: 'turkey').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'costa_rica').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.create(name: "Caribbean", slug: 'caribbean', :location_type => :country)
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o.locations << Location.where(slug: 'bolivia').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.where(slug: 'chile').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'paraguay').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'germany').limit(1)[0]
o = Organization.create(name: "Global Integrity", slug: "global_integrity", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://www.globalintegrity.org/")

o.sectors << Sector.where(slug: 'governance').limit(1)[0]
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o.locations << Location.where(slug: 'albania').limit(1)[0]
o.locations << Location.where(slug: 'argentina').limit(1)[0]
o.locations << Location.create(name: "Austria", slug: 'austria', :location_type => :country)
o.locations << Location.where(slug: 'algeria').limit(1)[0]
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.where(slug: 'azerbaijan').limit(1)[0]
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.where(slug: 'australia').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'belarus').limit(1)[0]
o.locations << Location.where(slug: 'bolivia').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'belgium').limit(1)[0]
o.locations << Location.where(slug: 'bosnia_and_herzegovina').limit(1)[0]
o.locations << Location.where(slug: 'bulgaria').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.create(name: "Cape Verde", slug: 'cape_verde', :location_type => :country)
o.locations << Location.where(slug: 'chile').limit(1)[0]
o.locations << Location.create(name: "Comoros", slug: 'comoros', :location_type => :country)
o.locations << Location.create(name: "Croatia", slug: 'croatia', :location_type => :country)
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.create(name: "Central African Republic (CAR)", slug: 'central_african_republic_car', :location_type => :country)
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'costa_rica').limit(1)[0]
o.locations << Location.create(name: "Czech Republic", slug: 'czech_republic', :location_type => :country)
o.locations << Location.where(slug: 'canada').limit(1)[0]
o.locations << Location.where(slug: 'chad').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'cote_divoire').limit(1)[0]
o.locations << Location.create(name: "Democratic Republic of Congo (DRC)", slug: 'democratic_republic_of_congo_drc', :location_type => :country)
o.locations << Location.create(name: "Djibouti", slug: 'djibouti', :location_type => :country)
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.create(name: "Eritrea", slug: 'eritrea', :location_type => :country)
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'equatorial_guinea').limit(1)[0]
o.locations << Location.where(slug: 'fiji').limit(1)[0]
o.locations << Location.where(slug: 'france').limit(1)[0]
o.locations << Location.create(name: "Gabon", slug: 'gabon', :location_type => :country)
o.locations << Location.where(slug: 'germany').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'gambia').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.create(name: "Guinea Bissau", slug: 'guinea_bissau', :location_type => :country)
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.create(name: "Hungary", slug: 'hungary', :location_type => :country)
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.create(name: "Ireland", slug: 'ireland', :location_type => :country)
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'israel').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.create(name: "Italy", slug: 'italy', :location_type => :country)
o.locations << Location.create(name: "Japan", slug: 'japan', :location_type => :country)
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'kazakhstan').limit(1)[0]
o.locations << Location.create(name: "Kuwait", slug: 'kuwait', :location_type => :country)
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.create(name: "Kyrgyz Republic", slug: 'kyrgyz_republic', :location_type => :country)
o.locations << Location.where(slug: 'kosovo').limit(1)[0]
o.locations << Location.create(name: "Latvia", slug: 'latvia', :location_type => :country)
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'libya').limit(1)[0]
o.locations << Location.where(slug: 'lesotho').limit(1)[0]
o.locations << Location.create(name: "Lithuania", slug: 'lithuania', :location_type => :country)
o.locations << Location.create(name: "Macedonia (FYROM)", slug: 'macedonia_fyrom', :location_type => :country)
o.locations << Location.where(slug: 'malaysia').limit(1)[0]
o.locations << Location.create(name: "Mauritius", slug: 'mauritius', :location_type => :country)
o.locations << Location.where(slug: 'mongolia').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'montenegro').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'mauritania').limit(1)[0]
o.locations << Location.where(slug: 'moldova').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'namibia').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'paraguay').limit(1)[0]
o.locations << Location.create(name: "Poland", slug: 'poland', :location_type => :country)
o.locations << Location.where(slug: 'panama').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'papua_new_guinea').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.create(name: "Qatar", slug: 'qatar', :location_type => :country)
o.locations << Location.where(slug: 'republic_of_the_congo').limit(1)[0]
o.locations << Location.where(slug: 'rwanda').limit(1)[0]
o.locations << Location.where(slug: 'romania').limit(1)[0]
o.locations << Location.create(name: "Russian Federation", slug: 'russian_federation', :location_type => :country)
o.locations << Location.create(name: "Sao Tome and Principe", slug: 'sao_tome_and_principe', :location_type => :country)
o.locations << Location.where(slug: 'seychelles').limit(1)[0]
o.locations << Location.where(slug: 'slovenia').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.create(name: "Spain", slug: 'spain', :location_type => :country)
o.locations << Location.where(slug: 'swaziland').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.create(name: "Solomon Islands", slug: 'solomon_islands', :location_type => :country)
o.locations << Location.create(name: "South Korea", slug: 'south_korea', :location_type => :country)
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'sweden').limit(1)[0]
o.locations << Location.where(slug: 'serbia').limit(1)[0]
o.locations << Location.create(name: "Slovak Republic", slug: 'slovak_republic', :location_type => :country)
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.create(name: "Syrian Arab Republic", slug: 'syrian_arab_republic', :location_type => :country)
o.locations << Location.where(slug: 'tajikistan').limit(1)[0]
o.locations << Location.create(name: "Timor Leste", slug: 'timor_leste', :location_type => :country)
o.locations << Location.create(name: "Trinidad and Tobago", slug: 'trinidad_and_tobago', :location_type => :country)
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'togo').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.create(name: "Tonga", slug: 'tonga', :location_type => :country)
o.locations << Location.where(slug: 'turkey').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.create(name: "United Kingdom (UK)", slug: 'united_kingdom_uk', :location_type => :country)
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.create(name: "United States (US)", slug: 'united_states_us', :location_type => :country)
o.locations << Location.create(name: "United Arab Emirates (UAE)", slug: 'united_arab_emirates_uae', :location_type => :country)
o.locations << Location.create(name: "Uruguay", slug: 'uruguay', :location_type => :country)
o.locations << Location.create(name: "Vanuatu", slug: 'vanuatu', :location_type => :country)
o.locations << Location.where(slug: 'venezuela').limit(1)[0]
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o.locations << Location.create(name: "West Bank", slug: 'west_bank', :location_type => :country)
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o = Organization.create(name: "Good Business", slug: "good_business", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.good.business")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'environment').limit(1)[0]
o.locations << Location.where(slug: 'london_uk').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.create(name: "Europe (UK?)", slug: 'europe_uk', :location_type => :country)
o = Organization.create(name: "Grameen Foundation", slug: "grameen_foundation", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.grameenfoundation.org/")

o.sectors << Sector.where(slug: 'finance').limit(1)[0]
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.create(name: "Turkey and other Middle Eastern countries", slug: 'turkey_and_other_middle_eastern', :location_type => :country)
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o = Organization.create(name: "Hack 4 Climate", slug: "hack__climate", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "http://cleantech21.org")

l = Location.create(name: "Zurich, Switzerland", slug: 'zurich_switzerland', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (47.3768866, 8.541694) ] WHERE slug = 'zurich_switzerland'")
o.locations << l
o.locations << Location.where(slug: 'switzerland').limit(1)[0]
o = Organization.create(name: "HealthEnabled", slug: "healthenabled", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://healthenabled.org/")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
l = Location.create(name: "Cape Town, South Africa", slug: 'cape_town_south_africa', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (-33.9248685, 18.4240553) ] WHERE slug = 'cape_town_south_africa'")
o.locations << l
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o = Organization.create(name: "Human Network International (HNI)", slug: "human_network_international_hni", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://hni.org/")

o.sectors << Sector.where(slug: 'ictd').limit(1)[0]
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'united_kingdom').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o = Organization.create(name: "Humanitarian Open Street Map Team ", slug: "humanitarian_open_street_map_tea", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "")

o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o = Organization.create(name: "InSTEDD", slug: "instedd", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://instedd.org/")

o.sectors << Sector.where(slug: 'ictd').limit(1)[0]
l = Location.create(name: "Sunnyvale, CA, USA", slug: 'sunnyvale_ca_usa', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (37.36883, -122.0363496) ] WHERE slug = 'sunnyvale_ca_usa'")
o.locations << l
o.locations << Location.where(slug: 'africa').limit(1)[0]
o.locations << Location.create(name: "Asia", slug: 'asia', :location_type => :country)
o.locations << Location.where(slug: 'latin_america').limit(1)[0]
o.locations << Location.create(name: "North America", slug: 'north_america', :location_type => :country)
o.locations << Location.create(name: "(Countries not listed)", slug: 'countries_not_listed', :location_type => :country)
o = Organization.create(name: "Institute of Development Studies (IDS)", slug: "institute_of_development_studies", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.ids.ac.uk")

o = Organization.create(name: "Integrity Global", slug: "integrity_global", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.integrityglobal.com")

o.locations << Location.where(slug: 'united_kingdom').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'libya').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'oman').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o = Organization.create(name: "IntelliSOFT", slug: "intellisoft", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.intellisoftkenya.com")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
l = Location.create(name: "Nairobi, Kenya", slug: 'nairobi_kenya', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (-1.2920659, 36.8219462) ] WHERE slug = 'nairobi_kenya'")
o.locations << l
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o = Organization.create(name: "International Foundation for Electoral Systems (IFES)", slug: "international_foundation_for_ele", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.ifes.org/")

o.sectors << Sector.where(slug: 'governance').limit(1)[0]
l = Location.create(name: "Arlington, VA", slug: 'arlington_va', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (38.8816208, -77.09098089999999) ] WHERE slug = 'arlington_va'")
o.locations << l
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'albania').limit(1)[0]
o.locations << Location.where(slug: 'algeria').limit(1)[0]
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.create(name: "Antigua and Barbuda", slug: 'antigua_and_barbuda', :location_type => :country)
o.locations << Location.where(slug: 'argentina').limit(1)[0]
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.create(name: "Aruba", slug: 'aruba', :location_type => :country)
o.locations << Location.where(slug: 'australia').limit(1)[0]
o.locations << Location.where(slug: 'austria').limit(1)[0]
o.locations << Location.where(slug: 'azerbaijan').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.create(name: "Barbados", slug: 'barbados', :location_type => :country)
o.locations << Location.where(slug: 'belarus').limit(1)[0]
o.locations << Location.where(slug: 'belgium').limit(1)[0]
o.locations << Location.where(slug: 'belize').limit(1)[0]
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.create(name: "Bermuda", slug: 'bermuda', :location_type => :country)
o.locations << Location.where(slug: 'bolivia').limit(1)[0]
o.locations << Location.where(slug: 'bosnia_and_herzegovina').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.create(name: "British Virgin Islands", slug: 'british_virgin_islands', :location_type => :country)
o.locations << Location.where(slug: 'bulgaria').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.where(slug: 'canada').limit(1)[0]
o.locations << Location.where(slug: 'cape_verde').limit(1)[0]
o.locations << Location.create(name: "Cayman Islands", slug: 'cayman_islands', :location_type => :country)
o.locations << Location.where(slug: 'central_african_republic').limit(1)[0]
o.locations << Location.where(slug: 'chad').limit(1)[0]
o.locations << Location.where(slug: 'chile').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'comoros').limit(1)[0]
o.locations << Location.where(slug: 'costa_rica').limit(1)[0]
o.locations << Location.create(name: "Côte d’Ivoire", slug: 'cte_divoire', :location_type => :country)
o.locations << Location.where(slug: 'croatia').limit(1)[0]
o.locations << Location.where(slug: 'cuba').limit(1)[0]
o.locations << Location.create(name: "Cyprus", slug: 'cyprus', :location_type => :country)
o.locations << Location.where(slug: 'czech_republic').limit(1)[0]
o.locations << Location.create(name: "Democratic Republic of Congo", slug: 'democratic_republic_of_congo', :location_type => :country)
o.locations << Location.create(name: "Denmark", slug: 'denmark', :location_type => :country)
o.locations << Location.where(slug: 'djibouti').limit(1)[0]
o.locations << Location.create(name: "Dominica Dominican Republic", slug: 'dominica_dominican_republic', :location_type => :country)
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'equatorial_guinea').limit(1)[0]
o.locations << Location.create(name: "Estonia", slug: 'estonia', :location_type => :country)
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'fiji').limit(1)[0]
o.locations << Location.create(name: "Finland", slug: 'finland', :location_type => :country)
o.locations << Location.where(slug: 'france').limit(1)[0]
o.locations << Location.where(slug: 'gabon').limit(1)[0]
o.locations << Location.where(slug: 'gambia').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'germany').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'greece').limit(1)[0]
o.locations << Location.create(name: "Grenada", slug: 'grenada', :location_type => :country)
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'guineabissau').limit(1)[0]
o.locations << Location.where(slug: 'guyana').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'hong_kong').limit(1)[0]
o.locations << Location.where(slug: 'hungary').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.create(name: "Iran", slug: 'iran', :location_type => :country)
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'italy').limit(1)[0]
o.locations << Location.where(slug: 'jamaica').limit(1)[0]
o.locations << Location.where(slug: 'japan').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'kazakhstan').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'kosovo').limit(1)[0]
o.locations << Location.where(slug: 'kyrgyzstan').limit(1)[0]
o.locations << Location.where(slug: 'laos').limit(1)[0]
o.locations << Location.where(slug: 'latvia').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'lesotho').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'libya').limit(1)[0]
o.locations << Location.where(slug: 'lithuania').limit(1)[0]
o.locations << Location.create(name: "Macao", slug: 'macao', :location_type => :country)
o.locations << Location.where(slug: 'macedonia').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'malaysia').limit(1)[0]
o.locations << Location.where(slug: 'maldives').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.create(name: "Malta", slug: 'malta', :location_type => :country)
o.locations << Location.create(name: "Marshall Islands", slug: 'marshall_islands', :location_type => :country)
o.locations << Location.where(slug: 'mauritania').limit(1)[0]
o.locations << Location.where(slug: 'mauritius').limit(1)[0]
o.locations << Location.create(name: "MENA", slug: 'mena', :location_type => :country)
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'micronesia').limit(1)[0]
o.locations << Location.where(slug: 'moldova').limit(1)[0]
o.locations << Location.create(name: "Monaco", slug: 'monaco', :location_type => :country)
o.locations << Location.where(slug: 'mongolia').limit(1)[0]
o.locations << Location.where(slug: 'montenegro').limit(1)[0]
o.locations << Location.create(name: "Montserrat", slug: 'montserrat', :location_type => :country)
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'namibia').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'netherlands').limit(1)[0]
o.locations << Location.create(name: "New Zealand", slug: 'new_zealand', :location_type => :country)
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.create(name: "Norfolk Island", slug: 'norfolk_island', :location_type => :country)
o.locations << Location.create(name: "Northern Mariana Islands", slug: 'northern_mariana_islands', :location_type => :country)
o.locations << Location.where(slug: 'norway').limit(1)[0]
o.locations << Location.where(slug: 'oman').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'panama').limit(1)[0]
o.locations << Location.where(slug: 'papua_new_guinea').limit(1)[0]
o.locations << Location.where(slug: 'paraguay').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'poland').limit(1)[0]
o.locations << Location.where(slug: 'portugal').limit(1)[0]
o.locations << Location.create(name: "Puerto Rico", slug: 'puerto_rico', :location_type => :country)
o.locations << Location.where(slug: 'qatar').limit(1)[0]
o.locations << Location.where(slug: 'romania').limit(1)[0]
o.locations << Location.where(slug: 'russian_federation').limit(1)[0]
o.locations << Location.where(slug: 'rwanda').limit(1)[0]
o.locations << Location.where(slug: 'saint_kitts_and_nevis').limit(1)[0]
o.locations << Location.where(slug: 'saint_lucia').limit(1)[0]
o.locations << Location.create(name: "Saint Vincent and the Grenadines", slug: 'saint_vincent_and_the_grenadines', :location_type => :country)
o.locations << Location.create(name: "Samoa", slug: 'samoa', :location_type => :country)
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'serbia').limit(1)[0]
o.locations << Location.where(slug: 'seychelles').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'singapore').limit(1)[0]
o.locations << Location.create(name: "Slovakia", slug: 'slovakia', :location_type => :country)
o.locations << Location.where(slug: 'slovenia').limit(1)[0]
o.locations << Location.where(slug: 'solomon_islands').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'spain').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.create(name: "Suriname", slug: 'suriname', :location_type => :country)
o.locations << Location.where(slug: 'swaziland').limit(1)[0]
o.locations << Location.where(slug: 'sweden').limit(1)[0]
o.locations << Location.where(slug: 'switzerland').limit(1)[0]
o.locations << Location.where(slug: 'syria').limit(1)[0]
o.locations << Location.create(name: "Taiwan", slug: 'taiwan', :location_type => :country)
o.locations << Location.where(slug: 'tajikistan').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'timorleste').limit(1)[0]
o.locations << Location.where(slug: 'togo').limit(1)[0]
o.locations << Location.where(slug: 'tonga').limit(1)[0]
o.locations << Location.where(slug: 'trinidad_and_tobago').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'turkey').limit(1)[0]
o.locations << Location.where(slug: 'turkmenistan').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'united_arab_emirates').limit(1)[0]
o.locations << Location.where(slug: 'united_kingdom').limit(1)[0]
o.locations << Location.create(name: "United States of America", slug: 'united_states_of_america', :location_type => :country)
o.locations << Location.where(slug: 'uruguay').limit(1)[0]
o.locations << Location.create(name: "US Virgin Islands", slug: 'us_virgin_islands', :location_type => :country)
o.locations << Location.where(slug: 'uzbekistan').limit(1)[0]
o.locations << Location.where(slug: 'venezuela').limit(1)[0]
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o.locations << Location.create(name: "West Bank/Gaza", slug: 'west_bankgaza', :location_type => :country)
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o = Organization.create(name: "International Initiative for Impact Evaluation (3ie)", slug: "international_initiative_for_imp", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "http://3ieimpact.org/")

o.sectors << Sector.where(slug: 'governance').limit(1)[0]
o.locations << Location.where(slug: 'united_kingdom').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o = Organization.create(name: "International Rescue Committee (IRC)", slug: "international_rescue_committee_i", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://www.rescue.org/")

o.sectors << Sector.where(slug: 'humanitarian__emergency').limit(1)[0]
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.create(name: "Germnay", slug: 'germnay', :location_type => :country)
o.locations << Location.where(slug: 'serbia').limit(1)[0]
o.locations << Location.where(slug: 'italy').limit(1)[0]
o.locations << Location.where(slug: 'greece').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'syria').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'libya').limit(1)[0]
o.locations << Location.where(slug: 'chad').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'ivory_coast').limit(1)[0]
o.locations << Location.where(slug: 'central_african_republic').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o = Organization.create(name: "Internews", slug: "internews", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://www.internews.org/")

l = Location.create(name: "California, USA", slug: 'california_usa', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (36.778261, -119.4179324) ] WHERE slug = 'california_usa'")
o.locations << l
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o.locations << Location.where(slug: 'london_uk').limit(1)[0]
l = Location.create(name: "Paris, France", slug: 'paris_france', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (48.856614, 2.3522219) ] WHERE slug = 'paris_france'")
o.locations << l
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'argentina').limit(1)[0]
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.create(name: "Bosnia & Herzegovina", slug: 'bosnia__herzegovina', :location_type => :country)
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.where(slug: 'central_african_republic').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'cote_divoire').limit(1)[0]
o.locations << Location.where(slug: 'democratic_republic_of_congo').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'fiji').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'kazakhstan').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'kosovo').limit(1)[0]
o.locations << Location.where(slug: 'kyrgyzstan').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'macedonia').limit(1)[0]
o.locations << Location.where(slug: 'malaysia').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'moldova').limit(1)[0]
o.locations << Location.where(slug: 'montenegro').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'papua_new_guinea').limit(1)[0]
o.locations << Location.where(slug: 'paraguay').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'puerto_rico').limit(1)[0]
o.locations << Location.where(slug: 'samoa').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'serbia').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.create(name: "Somaliland", slug: 'somaliland', :location_type => :country)
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'suriname').limit(1)[0]
o.locations << Location.where(slug: 'tajikistan').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'united_kingdom').limit(1)[0]
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o.locations << Location.where(slug: 'vanuatu').limit(1)[0]
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o.locations << Location.where(slug: 'west_bankgaza').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o = Organization.create(name: "IntraHealth International", slug: "intrahealth_international", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://www.intrahealth.org/")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'namibia').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'rwanda').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'togo').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o.locations << Location.where(slug: 'dominican_republic').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'panama').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'tajikistan').limit(1)[0]
o.locations << Location.where(slug: 'west_bank').limit(1)[0]
o.locations << Location.where(slug: 'laos').limit(1)[0]
o = Organization.create(name: "IREX", slug: "irex", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://www.irex.org/")

o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.create(name: "Cabo Verde", slug: 'cabo_verde', :location_type => :country)
o.locations << Location.where(slug: 'central_african_republic').limit(1)[0]
o.locations << Location.where(slug: 'chad').limit(1)[0]
o.locations << Location.where(slug: 'comoros').limit(1)[0]
o.locations << Location.where(slug: 'cte_divoire').limit(1)[0]
o.locations << Location.where(slug: 'democratic_republic_of_the_congo').limit(1)[0]
o.locations << Location.where(slug: 'djibouti').limit(1)[0]
o.locations << Location.where(slug: 'equatorial_guinea').limit(1)[0]
o.locations << Location.where(slug: 'eritrea').limit(1)[0]
o.locations << Location.create(name: "Eswatini", slug: 'eswatini', :location_type => :country)
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'gabon').limit(1)[0]
o.locations << Location.where(slug: 'gambia').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'guineabissau').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'lesotho').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'mauritania').limit(1)[0]
o.locations << Location.where(slug: 'mauritius').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'namibia').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'republic_of_the_congo').limit(1)[0]
o.locations << Location.where(slug: 'rwanda').limit(1)[0]
o.locations << Location.create(name: "São Tomé and Príncipe", slug: 'so_tom_and_prncipe', :location_type => :country)
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'seychelles').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'togo').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o.locations << Location.where(slug: 'argentina').limit(1)[0]
o.locations << Location.where(slug: 'bolivia').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.where(slug: 'chile').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'costa_rica').limit(1)[0]
o.locations << Location.where(slug: 'dominican_republic').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'panama').limit(1)[0]
o.locations << Location.where(slug: 'paraguay').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'trinidad_and_tobago').limit(1)[0]
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o.locations << Location.where(slug: 'uruguay').limit(1)[0]
o.locations << Location.where(slug: 'venezuela').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.create(name: "Burma", slug: 'burma', :location_type => :country)
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'laos').limit(1)[0]
o.locations << Location.where(slug: 'malaysia').limit(1)[0]
o.locations << Location.where(slug: 'maldives').limit(1)[0]
o.locations << Location.where(slug: 'mongolia').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.create(name: "Papua", slug: 'papua', :location_type => :country)
o.locations << Location.create(name: "New Guinea", slug: 'new_guinea', :location_type => :country)
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'singapore').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'timor_leste').limit(1)[0]
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o.locations << Location.where(slug: 'albania').limit(1)[0]
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.where(slug: 'azerbaijan').limit(1)[0]
o.locations << Location.where(slug: 'belarus').limit(1)[0]
o.locations << Location.where(slug: 'bosnia_and_herzegovina').limit(1)[0]
o.locations << Location.where(slug: 'bulgaria').limit(1)[0]
o.locations << Location.where(slug: 'croatia').limit(1)[0]
o.locations << Location.where(slug: 'czech_republic').limit(1)[0]
o.locations << Location.where(slug: 'estonia').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'hungary').limit(1)[0]
o.locations << Location.where(slug: 'kazakhstan').limit(1)[0]
o.locations << Location.where(slug: 'kosovo').limit(1)[0]
o.locations << Location.where(slug: 'kyrgyzstan').limit(1)[0]
o.locations << Location.where(slug: 'latvia').limit(1)[0]
o.locations << Location.where(slug: 'lithuania').limit(1)[0]
o.locations << Location.where(slug: 'macedonia').limit(1)[0]
o.locations << Location.where(slug: 'moldova').limit(1)[0]
o.locations << Location.where(slug: 'montenegro').limit(1)[0]
o.locations << Location.where(slug: 'poland').limit(1)[0]
o.locations << Location.where(slug: 'romania').limit(1)[0]
o.locations << Location.where(slug: 'russia').limit(1)[0]
o.locations << Location.where(slug: 'serbia').limit(1)[0]
o.locations << Location.where(slug: 'slovakia').limit(1)[0]
o.locations << Location.where(slug: 'slovenia').limit(1)[0]
o.locations << Location.where(slug: 'tajikistan').limit(1)[0]
o.locations << Location.where(slug: 'turkey').limit(1)[0]
o.locations << Location.where(slug: 'turkmenistan').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'uzbekistan').limit(1)[0]
o.locations << Location.where(slug: 'algeria').limit(1)[0]
o.locations << Location.where(slug: 'bahrain').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'israel').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'kuwait').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'libya').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'oman').limit(1)[0]
o.locations << Location.where(slug: 'west_bankgaza').limit(1)[0]
o.locations << Location.where(slug: 'qatar').limit(1)[0]
o.locations << Location.where(slug: 'saudi_arabia').limit(1)[0]
o.locations << Location.where(slug: 'syria').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'united_arab_emirates').limit(1)[0]
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o = Organization.create(name: "IST Uganda", slug: "ist_uganda", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "")

o = Organization.create(name: "iTech Mission Private Limited (iTM)", slug: "itech_mission_private_limited_it", when_endorsed: DateTime.new(2015, 1, 1), is_endorser: true, website: "http://www.itechmission.org/ ")

o.sectors << Sector.where(slug: 'socioeconomic').limit(1)[0]
o.locations << Location.where(slug: 'new_delhi_india').limit(1)[0]
o.locations << Location.create(name: "Over 100 Countries", slug: 'over__countries', :location_type => :country)
o = Organization.create(name: "Jhpiego", slug: "jhpiego", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://www.jhpiego.org/")

o.sectors << Sector.where(slug: 'gender').limit(1)[0]
o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.locations << Location.where(slug: 'baltimore_md').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.where(slug: 'chad').limit(1)[0]
o.locations << Location.where(slug: 'ivory_coast').limit(1)[0]
o.locations << Location.where(slug: 'drc').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'lesotho').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'namibia').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'togo').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o = Organization.create(name: "John Snow, Inc. (JSI)", slug: "john_snow_inc_jsi", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.jsi.com/")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
l = Location.create(name: "HQ: Boston, MA", slug: 'hq_boston_ma', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (42.328326, -71.07021999999999) ] WHERE slug = 'hq_boston_ma'")
o.locations << l
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o.locations << Location.where(slug: 'albania').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'romania').limit(1)[0]
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'russia').limit(1)[0]
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.where(slug: 'guineabissau').limit(1)[0]
o.locations << Location.where(slug: 'rwanda').limit(1)[0]
o.locations << Location.where(slug: 'azerbaijan').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'bolivia').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.where(slug: 'kazakhstan').limit(1)[0]
o.locations << Location.where(slug: 'swaziland').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'tajikistan').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'kyrgyz_republic').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'laos').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'timorleste').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'togo').limit(1)[0]
o.locations << Location.where(slug: 'congo').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'trinidad_and_tobago').limit(1)[0]
o.locations << Location.create(name: "Cote d'Ivoire Mali", slug: 'cote_divoire_mali', :location_type => :country)
o.locations << Location.where(slug: 'turkmenistan').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'djibouti').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'dominican_republic').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'uzbekistan').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'namibia').limit(1)[0]
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'west_bank').limit(1)[0]
o.locations << Location.where(slug: 'eritrea').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'gambia').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o = Organization.create(name: "Kimetrica", slug: "kimetrica", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "http://kimetrica.com")

o.sectors << Sector.where(slug: 'poverty').limit(1)[0]
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'drc').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'france').limit(1)[0]
o.locations << Location.where(slug: 'gambia').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'ivory_coast').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'laos').limit(1)[0]
o.locations << Location.where(slug: 'lesotho').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'mauritania').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'namibia').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 'swaziland').limit(1)[0]
o.locations << Location.where(slug: 'switzerland').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.create(name: "UK", slug: 'uk', :location_type => :country)
o.locations << Location.create(name: "USA", slug: 'usa', :location_type => :country)
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o = Organization.create(name: "Kingship Digital Inc. ", slug: "kingship_digital_inc", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.kingshipdigital.com")

o.sectors << Sector.where(slug: 'media_production').limit(1)[0]
o.sectors << Sector.where(slug: 'consulting').limit(1)[0]
l = Location.create(name: "Kampala, Uganda", slug: 'kampala_uganda', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (0.3475964, 32.5825197) ] WHERE slug = 'kampala_uganda'")
o.locations << l
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o = Organization.create(name: "LINGOs", slug: "lingos", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://lingos.org/")

o.locations << Location.create(name: "Members are organizations", slug: 'members_are_organizations', :location_type => :country)
o = Organization.create(name: "Logical Outcomes", slug: "logical_outcomes", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "http://www.logicaloutcomes.net/")

l = Location.create(name: "Toronto, Canada", slug: 'toronto_canada', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (43.653226, -79.3831843) ] WHERE slug = 'toronto_canada'")
o.locations << l
o.locations << Location.create(name: "50 Nations", slug: '_nations', :location_type => :country)
o = Organization.create(name: "Lutheran World Relief", slug: "lutheran_world_relief", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "programs.lwr.org")

o.sectors << Sector.where(slug: 'agriculture').limit(1)[0]
o.sectors << Sector.where(slug: 'emergency').limit(1)[0]
o.sectors << Sector.where(slug: 'climate').limit(1)[0]
o.locations << Location.where(slug: 'baltimore_md').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'syria').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'palestine').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.create(name: "the Philippines", slug: 'the_philippines', :location_type => :country)
o = Organization.create(name: "Main Level", slug: "main_level", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.mainlevel.de")

o.sectors << Sector.where(slug: 'consulting').limit(1)[0]
l = Location.create(name: "Eschborn, Germany", slug: 'eschborn_germany', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (50.1467469, 8.561455500000001) ] WHERE slug = 'eschborn_germany'")
o.locations << l
o.locations << Location.where(slug: 'usa').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'uruguay').limit(1)[0]
o.locations << Location.where(slug: 'argentina').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'lesotho').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.create(name: "Occupied Palestinian Territory", slug: 'occupied_palestinian_territory', :location_type => :country)
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'belgium').limit(1)[0]
o.locations << Location.where(slug: 'germany').limit(1)[0]
o.locations << Location.where(slug: 'switzerland').limit(1)[0]
o.locations << Location.where(slug: 'austria').limit(1)[0]
o.locations << Location.where(slug: 'kosovo').limit(1)[0]
o.locations << Location.where(slug: 'romania').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'russia').limit(1)[0]
o.locations << Location.where(slug: 'mongolia').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o = Organization.create(name: "Makaia", slug: "makaia", when_endorsed: DateTime.new(2017, 1, 1), is_endorser: true, website: "http://makaia.org/en/home/")

l = Location.create(name: "Medellín, Colombia.", slug: 'medelln_colombia', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (6.244203, -75.5812119) ] WHERE slug = 'medelln_colombia'")
o.locations << l
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o = Organization.create(name: "Medic Mobile", slug: "medic_mobile", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://medicmobile.org/")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'emergency').limit(1)[0]
l = Location.create(name: "San Francisco, CA, USA", slug: 'san_francisco_ca_usa', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (37.7749295, -122.4194155) ] WHERE slug = 'san_francisco_ca_usa'")
o.locations << l
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'mauritania').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o = Organization.create(name: "Mercy Corps", slug: "mercy_corps", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://www.mercycorps.org")

o.sectors << Sector.where(slug: 'emergency').limit(1)[0]
l = Location.create(name: "Portland, OR, USA", slug: 'portland_or_usa', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (45.5122308, -122.6587185) ] WHERE slug = 'portland_or_usa'")
o.locations << l
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'central_african_republic').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'greece').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'kyrgystan').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'libya').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'mongolia').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 'syria').limit(1)[0]
o.locations << Location.where(slug: 'tajikistan').limit(1)[0]
o.locations << Location.where(slug: 'timorleste').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.create(name: "US", slug: 'us', :location_type => :country)
o.locations << Location.create(name: "West Bank & Gaza", slug: 'west_bank__gaza', :location_type => :country)
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o = Organization.create(name: "Moonshot Global Consulting", slug: "moonshot_global_consulting", when_endorsed: DateTime.new(2017, 1, 1), is_endorser: true, website: "http://moonshotglobal.com/")

o.sectors << Sector.where(slug: 'ictd').limit(1)[0]
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o.locations << Location.where(slug: 'africa').limit(1)[0]
o.locations << Location.where(slug: 'europe').limit(1)[0]
o.locations << Location.create(name: "South America", slug: 'south_america', :location_type => :country)
o.locations << Location.create(name: "Southeast Asia", slug: 'southeast_asia', :location_type => :country)
o.locations << Location.create(name: "and the United States (Network of Consultants)", slug: 'and_the_united_states_network_of', :location_type => :country)
o = Organization.create(name: "mPower Social Enterprises Ltd.", slug: "mpower_social_enterprises_ltd", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://www.mpower-social.com/")

o.sectors << Sector.where(slug: 'ictd').limit(1)[0]
l = Location.create(name: "Dhaka, Bangladesh", slug: 'dhaka_bangladesh', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (23.810332, 90.4125181) ] WHERE slug = 'dhaka_bangladesh'")
o.locations << l
o.locations << Location.where(slug: 'usa').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o = Organization.create(name: "mPowering Frontline Health Workers", slug: "mpowering_frontline_health_worke", when_endorsed: DateTime.new(2015, 1, 1), is_endorser: true, website: "http://mpoweringhealth.org/")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o.locations << Location.create(name: "Platform used in North America", slug: 'platform_used_in_north_america', :location_type => :country)
o.locations << Location.where(slug: 'south_america').limit(1)[0]
o.locations << Location.where(slug: 'africa').limit(1)[0]
o.locations << Location.where(slug: 'asia').limit(1)[0]
o.locations << Location.where(slug: 'europe').limit(1)[0]
o.locations << Location.where(slug: 'oceania').limit(1)[0]
o = Organization.create(name: "Nafundi", slug: "nafundi", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://nafundi.com/")

o.sectors << Sector.where(slug: 'ictd').limit(1)[0]
l = Location.create(name: "San Diego, CA, USA", slug: 'san_diego_ca_usa', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (32.715738, -117.1610838) ] WHERE slug = 'san_diego_ca_usa'")
o.locations << l
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.create(name: "ISS", slug: 'iss', :location_type => :country)
o = Organization.create(name: "National Democratic Institute (NDI)", slug: "national_democratic_institute_nd", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://www.ndi.org/")

o.sectors << Sector.where(slug: 'governance').limit(1)[0]
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'cuba').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'venezuela').limit(1)[0]
o.locations << Location.where(slug: 'albania').limit(1)[0]
o.locations << Location.where(slug: 'bosnia__herz').limit(1)[0]
o.locations << Location.where(slug: 'bulgaria').limit(1)[0]
o.locations << Location.where(slug: 'croatia').limit(1)[0]
o.locations << Location.where(slug: 'hungary').limit(1)[0]
o.locations << Location.where(slug: 'kosovo').limit(1)[0]
o.locations << Location.where(slug: 'macedonia').limit(1)[0]
o.locations << Location.where(slug: 'montenegro').limit(1)[0]
o.locations << Location.where(slug: 'poland').limit(1)[0]
o.locations << Location.where(slug: 'romania').limit(1)[0]
o.locations << Location.where(slug: 'serbia').limit(1)[0]
o.locations << Location.where(slug: 'slovakia').limit(1)[0]
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.where(slug: 'azerbaijan').limit(1)[0]
o.locations << Location.where(slug: 'belarus').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'kyrgyzstan').limit(1)[0]
o.locations << Location.where(slug: 'moldova').limit(1)[0]
o.locations << Location.where(slug: 'russia').limit(1)[0]
o.locations << Location.where(slug: 'turkey').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'hong_kong').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.create(name: "North Korea", slug: 'north_korea', :location_type => :country)
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'timorleste').limit(1)[0]
o = Organization.create(name: "Norwegian People''s Aid", slug: "norwegian_peoples_aid", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "http://www.npaid.org")

o.sectors << Sector.where(slug: 'gender').limit(1)[0]
o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'humanitarian').limit(1)[0]
o.sectors << Sector.where(slug: 'governance_').limit(1)[0]
l = Location.create(name: "Stockholm, Sweden", slug: 'stockholm_sweden', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (59.32932349999999, 18.0685808) ] WHERE slug = 'stockholm_sweden'")
o.locations << l
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'cuba').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'syria').limit(1)[0]
o.locations << Location.where(slug: 'palestine').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o = Organization.create(name: "Ona", slug: "ona", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://ona.io/home/")

o.sectors << Sector.where(slug: 'ictd').limit(1)[0]
o = Organization.create(name: "Open Data Kit", slug: "open_data_kit", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://opendatakit.org/")

o.sectors << Sector.where(slug: 'governance').limit(1)[0]
o = Organization.create(name: "OpenHIE", slug: "openhie", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://ohie.org/")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o = Organization.create(name: "OpenLMIS", slug: "openlmis", when_endorsed: DateTime.new(2017, 1, 1), is_endorser: true, website: "http://openlmis.org/")

o.sectors << Sector.where(slug: 'ictd').limit(1)[0]
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'ivory_coast').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o = Organization.create(name: "OpenMRS", slug: "openmrs", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://openmrs.org/")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
l = Location.create(name: "Carmel, IN, USA", slug: 'carmel_in_usa', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (39.978371, -86.1180435) ] WHERE slug = 'carmel_in_usa'")
o.locations << l
o.locations << Location.where(slug: 'canada').limit(1)[0]
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'puerto_rico').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'suriname').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'argentina').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.where(slug: 'uk').limit(1)[0]
o.locations << Location.where(slug: 'france').limit(1)[0]
o.locations << Location.where(slug: 'germany').limit(1)[0]
o.locations << Location.where(slug: 'spain').limit(1)[0]
o.locations << Location.where(slug: 'romania').limit(1)[0]
o.locations << Location.where(slug: 'greece').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'algeria').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'kazakhstan').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.create(name: "Austrailia", slug: 'austrailia', :location_type => :country)
o.locations << Location.where(slug: 'north_korea').limit(1)[0]
o = Organization.create(name: "OpenWise", slug: "openwise", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.openwise.co/")

o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o = Organization.create(name: "Oxfam", slug: "oxfam", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "https://www.oxfam.org/ ")

o.sectors << Sector.where(slug: 'poverty').limit(1)[0]
l = Location.create(name: "Boston, MA, USA", slug: 'boston_ma_usa', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (42.3600825, -71.0588801) ] WHERE slug = 'boston_ma_usa'")
o.locations << l
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'albania').limit(1)[0]
o.locations << Location.where(slug: 'algeria').limit(1)[0]
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.where(slug: 'australia').limit(1)[0]
o.locations << Location.where(slug: 'azerbaijan').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'barbados').limit(1)[0]
o.locations << Location.where(slug: 'belgium').limit(1)[0]
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'bolivia').limit(1)[0]
o.locations << Location.where(slug: 'bosnia_and_herzegovina').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'canada').limit(1)[0]
o.locations << Location.where(slug: 'central_african_republic').limit(1)[0]
o.locations << Location.where(slug: 'chad').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'cte_divoire').limit(1)[0]
o.locations << Location.where(slug: 'cuba').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'denmark').limit(1)[0]
o.locations << Location.where(slug: 'dominican_republic').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'fiji').limit(1)[0]
o.locations << Location.where(slug: 'france').limit(1)[0]
o.locations << Location.where(slug: 'gambia').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'germany').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'greece').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.create(name: "Bissau", slug: 'bissau', :location_type => :country)
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'ireland').limit(1)[0]
o.locations << Location.where(slug: 'italy').limit(1)[0]
o.locations << Location.where(slug: 'japan').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'laos').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'malaysia').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'mauritania').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'netherlands').limit(1)[0]
o.locations << Location.where(slug: 'new_zealand').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'north_korea').limit(1)[0]
o.locations << Location.where(slug: 'occupied_palestinian_territory').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.create(name: "Papua New", slug: 'papua_new', :location_type => :country)
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'paraguay').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'russian_federation').limit(1)[0]
o.locations << Location.where(slug: 'rwanda').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'solomon_islands').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'spain').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 'syria').limit(1)[0]
o.locations << Location.where(slug: 'tajikistan').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'timorleste_east_timor').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'turkey').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'united_kingdom').limit(1)[0]
o.locations << Location.where(slug: 'usa').limit(1)[0]
o.locations << Location.where(slug: 'vanuatu').limit(1)[0]
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o.locations << Location.create(name: "Western Sahara", slug: 'western_sahara', :location_type => :country)
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o = Organization.create(name: "Palladium", slug: "palladium", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://thepalladiumgroup.com/")

o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'environment').limit(1)[0]
o.sectors << Sector.where(slug: 'work_force').limit(1)[0]
o.sectors << Sector.where(slug: 'governance').limit(1)[0]
o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'logisitcs').limit(1)[0]
o.locations << Location.where(slug: 'london_uk').limit(1)[0]
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
l = Location.create(name: "Brisbane, Austrailia", slug: 'brisbane_austrailia', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (-27.4697707, 153.0251235) ] WHERE slug = 'brisbane_austrailia'")
o.locations << l
l = Location.create(name: "Dubai, United Arab Emirates", slug: 'dubai_united_arab_emirates', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (25.2048493, 55.2707828) ] WHERE slug = 'dubai_united_arab_emirates'")
o.locations << l
l = Location.create(name: "Abuja, Nigeria", slug: 'abuja_nigeria', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (9.0764785, 7.398574) ] WHERE slug = 'abuja_nigeria'")
o.locations << l
l = Location.create(name: "NY, NY", slug: 'ny_ny', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (40.7127753, -74.0059728) ] WHERE slug = 'ny_ny'")
o.locations << l
l = Location.create(name: "Canberra, Austrailia", slug: 'canberra_austrailia', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (-35.2809368, 149.1300092) ] WHERE slug = 'canberra_austrailia'")
o.locations << l
l = Location.create(name: "Jakarta, Indonesia", slug: 'jakarta_indonesia', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (-6.180495, 106.8283415) ] WHERE slug = 'jakarta_indonesia'")
o.locations << l
o.locations << Location.where(slug: 'nairobi_kenya').limit(1)[0]
l = Location.create(name: "Sydney, Austrailia", slug: 'sydney_austrailia', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (-33.8688197, 151.2092955) ] WHERE slug = 'sydney_austrailia'")
o.locations << l
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.create(name: "Andorra", slug: 'andorra', :location_type => :country)
o.locations << Location.create(name: "Anguilla", slug: 'anguilla', :location_type => :country)
o.locations << Location.where(slug: 'argentina').limit(1)[0]
o.locations << Location.where(slug: 'australia').limit(1)[0]
o.locations << Location.where(slug: 'azerbaijan').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'canada').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'congo').limit(1)[0]
o.locations << Location.where(slug: 'cote_divoire').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'fiji').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'guyana').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'jamaica').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'macedonia').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'mauritania').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.create(name: "Nauru", slug: 'nauru', :location_type => :country)
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'papua_new_guinea').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'qatar').limit(1)[0]
o.locations << Location.where(slug: 'rwanda').limit(1)[0]
o.locations << Location.where(slug: 'saudi_arabia').limit(1)[0]
o.locations << Location.where(slug: 'singapore').limit(1)[0]
o.locations << Location.where(slug: 'solomon_islands').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'spain').limit(1)[0]
o.locations << Location.where(slug: 'sweden').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.create(name: "The Netherlands", slug: 'the_netherlands', :location_type => :country)
o.locations << Location.where(slug: 'timorleste').limit(1)[0]
o.locations << Location.where(slug: 'togo').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'united_arab_emirates').limit(1)[0]
o.locations << Location.where(slug: 'united_kingdom').limit(1)[0]
o.locations << Location.where(slug: 'usa').limit(1)[0]
o.locations << Location.where(slug: 'vanuatu').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o = Organization.create(name: "PATH", slug: "path", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.path.org/")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o = Organization.create(name: "Pathfinder International", slug: "pathfinder_international", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.pathfinder.org/")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
l = Location.create(name: "Watertown, MA, USA", slug: 'watertown_ma_usa', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (42.3709299, -71.1828321) ] WHERE slug = 'watertown_ma_usa'")
o.locations << l
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'ivory_coast').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'togo').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o = Organization.create(name: "People in Need", slug: "people_in_need", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.peopleinneed.cz")

o.sectors << Sector.where(slug: 'humanitarian_emergency').limit(1)[0]
l = Location.create(name: "Prague, Czech Republic", slug: 'prague_czech_republic', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (50.0755381, 14.4378005) ] WHERE slug = 'prague_czech_republic'")
o.locations << l
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'bosnia_and_herz').limit(1)[0]
o.locations << Location.create(name: "Burma/ Myanmar", slug: 'burma_myanmar', :location_type => :country)
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'czech_republic').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.create(name: "Irag", slug: 'irag', :location_type => :country)
o.locations << Location.where(slug: 'kosovo').limit(1)[0]
o.locations << Location.where(slug: 'moldova').limit(1)[0]
o.locations << Location.where(slug: 'mongolia').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'romania').limit(1)[0]
o.locations << Location.where(slug: 'serbia').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'syria').limit(1)[0]
o.locations << Location.where(slug: 'turkey').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o = Organization.create(name: "Plan International", slug: "plan_international", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "http://plan-international.org")

o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'youth').limit(1)[0]
o.sectors << Sector.where(slug: 'gender').limit(1)[0]
l = Location.create(name: "Dukes Court, UK", slug: 'dukes_court_uk', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (51.3206581, -0.554314) ] WHERE slug = 'dukes_court_uk'")
o.locations << l
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.where(slug: 'central_african_republic').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.create(name: "Guinea - Bassu", slug: 'guinea__bassu', :location_type => :country)
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'togo').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o.locations << Location.where(slug: 'bolivia').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'dominican_republic').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'paraguay').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'laos').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'timorleste').limit(1)[0]
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o.locations << Location.where(slug: 'austrailia').limit(1)[0]
o.locations << Location.create(name: "Belgium (Flemish)", slug: 'belgium_flemish', :location_type => :country)
o.locations << Location.create(name: "Belgium (French)", slug: 'belgium_french', :location_type => :country)
o.locations << Location.create(name: "Canada (English)", slug: 'canada_english', :location_type => :country)
o.locations << Location.create(name: "Canada (French)", slug: 'canada_french', :location_type => :country)
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'denmark').limit(1)[0]
o.locations << Location.where(slug: 'finland').limit(1)[0]
o.locations << Location.where(slug: 'france').limit(1)[0]
o.locations << Location.where(slug: 'germany').limit(1)[0]
o.locations << Location.create(name: "Hong King", slug: 'hong_king', :location_type => :country)
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'ireland').limit(1)[0]
o.locations << Location.where(slug: 'italy').limit(1)[0]
o.locations << Location.where(slug: 'japan').limit(1)[0]
o.locations << Location.create(name: "Korea", slug: 'korea', :location_type => :country)
o.locations << Location.where(slug: 'netherlands').limit(1)[0]
o.locations << Location.where(slug: 'norway').limit(1)[0]
o.locations << Location.where(slug: 'spain').limit(1)[0]
o.locations << Location.where(slug: 'sweden').limit(1)[0]
o.locations << Location.create(name: "Switzerland (French)", slug: 'switzerland_french', :location_type => :country)
o.locations << Location.create(name: "Switzerland (German)", slug: 'switzerland_german', :location_type => :country)
o.locations << Location.where(slug: 'uk').limit(1)[0]
o.locations << Location.where(slug: 'usa').limit(1)[0]
o = Organization.create(name: "Praekelt.org", slug: "praekeltorg", when_endorsed: DateTime.new(2015, 1, 1), is_endorser: true, website: "http://www.praekelt.org/")

o.sectors << Sector.where(slug: 'ictd').limit(1)[0]
o.locations << Location.create(name: "Wokring with Girl Effect in over 65 countries", slug: 'wokring_with_girl_effect_in_over', :location_type => :country)
o.locations << Location.create(name: "Working in countries in Sub-Saharan Africa and South Africa", slug: 'working_in_countries_in_subsahar', :location_type => :country)
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o = Organization.create(name: "Reboot", slug: "reboot", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://reboot.org/")

o.sectors << Sector.where(slug: 'governance').limit(1)[0]
l = Location.create(name: "NYC, USA", slug: 'nyc_usa', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (40.7127753, -74.0059728) ] WHERE slug = 'nyc_usa'")
o.locations << l
o.locations << Location.where(slug: 'canada').limit(1)[0]
o.locations << Location.where(slug: 'us').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'panama').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.where(slug: 'uk').limit(1)[0]
o.locations << Location.create(name: "Natherlands", slug: 'natherlands', :location_type => :country)
o.locations << Location.where(slug: 'czech_republic').limit(1)[0]
o.locations << Location.create(name: "Moldova Turkey", slug: 'moldova_turkey', :location_type => :country)
o.locations << Location.create(name: "Gerogia", slug: 'gerogia', :location_type => :country)
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'libya').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'south_korea').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o = Organization.create(name: "Relief Applications", slug: "relief_applications", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "http://reliefapplications.org")

o.sectors << Sector.where(slug: 'ictd').limit(1)[0]
l = Location.create(name: "Madrid, Spain", slug: 'madrid_spain', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (40.4167754, -3.7037902) ] WHERE slug = 'madrid_spain'")
o.locations << l
o.locations << Location.where(slug: 'us').limit(1)[0]
o.locations << Location.where(slug: 'costa_rica').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'uk').limit(1)[0]
o.locations << Location.where(slug: 'france').limit(1)[0]
o.locations << Location.where(slug: 'spain').limit(1)[0]
o.locations << Location.where(slug: 'italy').limit(1)[0]
o.locations << Location.where(slug: 'mauritania').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'democratic_republic_of_congo').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o = Organization.create(name: "Resolve to Save Lives", slug: "resolve_to_save_lives", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "resolvetosavelives.org")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.locations << Location.where(slug: 'nyc_usa').limit(1)[0]
o = Organization.create(name: "RTI International", slug: "rti_international", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.rti.org/")

o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'workforce_development').limit(1)[0]
o.sectors << Sector.where(slug: 'energy').limit(1)[0]
o.sectors << Sector.where(slug: 'environment').limit(1)[0]
o.sectors << Sector.where(slug: 'food_security_and_agriculture').limit(1)[0]
o.sectors << Sector.where(slug: 'social_and_justice_poilcy').limit(1)[0]
o.sectors << Sector.where(slug: 'innovation_ecosystem').limit(1)[0]
l = Location.create(name: "Research Triangle Park, NC, USA", slug: 'research_triangle_park_nc_usa', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (35.8991678, -78.86364019999999) ] WHERE slug = 'research_triangle_park_nc_usa'")
o.locations << l
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'south_korea').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'sengal').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o.locations << Location.create(name: "UAE", slug: 'uae', :location_type => :country)
o.locations << Location.create(name: "Barabdos", slug: 'barabdos', :location_type => :country)
o.locations << Location.where(slug: 'belize').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.where(slug: 'dominican_republic').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'jamaica').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'suriname').limit(1)[0]
o.locations << Location.where(slug: 'uruguay').limit(1)[0]
o = Organization.create(name: "Search for Common Ground", slug: "search_for_common_ground", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.sfcg.org")

o.sectors << Sector.where(slug: 'conflict_sensativity').limit(1)[0]
o.sectors << Sector.where(slug: 'natural_resource_conflicts').limit(1)[0]
o.sectors << Sector.where(slug: 'disarmament').limit(1)[0]
o.sectors << Sector.where(slug: 'demobilization__reintegration').limit(1)[0]
o.sectors << Sector.where(slug: 'reconciliation').limit(1)[0]
o.sectors << Sector.where(slug: 'fair__responsibile_media').limit(1)[0]
o.sectors << Sector.where(slug: 'refugee__internal_displacement_i').limit(1)[0]
o.sectors << Sector.where(slug: 'gender_equality').limit(1)[0]
o.sectors << Sector.where(slug: 'security_sector_reform').limit(1)[0]
o.sectors << Sector.where(slug: 'governance').limit(1)[0]
o.sectors << Sector.where(slug: 'democracy__elections').limit(1)[0]
o.sectors << Sector.where(slug: 'sexual__gender_based_violence').limit(1)[0]
o.sectors << Sector.where(slug: 'religious_engagement').limit(1)[0]
o.sectors << Sector.where(slug: 'violent_exteremism').limit(1)[0]
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
l = Location.create(name: "Brussles, Belgium", slug: 'brussles_belgium', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (50.8503463, 4.3517211) ] WHERE slug = 'brussles_belgium'")
o.locations << l
o.locations << Location.where(slug: 'us').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'ivory_coast').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'chad').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 'central_african_republic').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'macedonia').limit(1)[0]
o.locations << Location.where(slug: 'uk').limit(1)[0]
o.locations << Location.where(slug: 'belgium').limit(1)[0]
o.locations << Location.where(slug: 'kyrgyzstan').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.create(name: "Indoonesia", slug: 'indoonesia', :location_type => :country)
o.locations << Location.create(name: "East Timor", slug: 'east_timor', :location_type => :country)
o = Organization.create(name: "SlashRoots Foundation", slug: "slashroots_foundation", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.slashroots.org/#/")

o.sectors << Sector.where(slug: 'ictd').limit(1)[0]
l = Location.create(name: "Kingston, Jamaica", slug: 'kingston_jamaica', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (18.0178743, -76.8099041) ] WHERE slug = 'kingston_jamaica'")
o.locations << l
o.locations << Location.create(name: "The Caribbean", slug: 'the_caribbean', :location_type => :country)
o = Organization.create(name: "SMART Resultancy", slug: "smart_resultancy", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.smartresultancy.nl/")

o.sectors << Sector.where(slug: 'ictd').limit(1)[0]
o.locations << Location.where(slug: 'netherlands').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'the_netherlands').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'togo').limit(1)[0]
o.locations << Location.create(name: "Uganda and Zambia.", slug: 'uganda_and_zambia', :location_type => :country)
o = Organization.create(name: "Somali Disaster Resilience Institute (SDRI)", slug: "somali_disaster_resilience_insti", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.sdri.so/")

o.sectors << Sector.where(slug: 'emergency').limit(1)[0]
l = Location.create(name: "Mogadishu, Somalia", slug: 'mogadishu_somalia', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (2.0469343, 45.3181623) ] WHERE slug = 'mogadishu_somalia'")
o.locations << l
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o = Organization.create(name: "Souktel", slug: "souktel", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.souktel.org/")

o.sectors << Sector.where(slug: 'ictd').limit(1)[0]
o.sectors << Sector.where(slug: 'education__youth').limit(1)[0]
o.sectors << Sector.where(slug: 'agriculture').limit(1)[0]
o.sectors << Sector.where(slug: 'democracy__governance').limit(1)[0]
o.sectors << Sector.where(slug: 'economic_growth__employment').limit(1)[0]
o.sectors << Sector.where(slug: 'emergency_response').limit(1)[0]
o.sectors << Sector.where(slug: 'women__gender_equality').limit(1)[0]
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
l = Location.create(name: "Ramallah, Palestine", slug: 'ramallah_palestine', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (31.9037641, 35.2034184) ] WHERE slug = 'ramallah_palestine'")
o.locations << l
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'libya').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'palestine').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'saudi_arabia').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 'suriname').limit(1)[0]
o.locations << Location.where(slug: 'syria').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'turkey').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'uae').limit(1)[0]
o.locations << Location.where(slug: 'vanuatu').limit(1)[0]
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o = Organization.create(name: "SurveyCTO", slug: "surveycto", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.surveycto.com/index.html")

o.sectors << Sector.where(slug: 'data_collection').limit(1)[0]
o = Organization.create(name: "Swedish International Development Cooperation Agency (SIDA)", slug: "swedish_international_developmen", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.sida.se/English/")

o.sectors << Sector.where(slug: 'humaitarian_aid').limit(1)[0]
o.sectors << Sector.where(slug: 'human_rights').limit(1)[0]
o.sectors << Sector.where(slug: 'gender_equality').limit(1)[0]
o.sectors << Sector.where(slug: 'climate').limit(1)[0]
o.sectors << Sector.where(slug: 'helath').limit(1)[0]
o.sectors << Sector.where(slug: 'market_development').limit(1)[0]
o.sectors << Sector.where(slug: 'agriculture_and_food_security').limit(1)[0]
o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'sustainable_social_development').limit(1)[0]
o.sectors << Sector.where(slug: 'conflict_resolution__peace__secu').limit(1)[0]
o.locations << Location.where(slug: 'stockholm_sweden').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o.locations << Location.create(name: "Middle East & North Africa", slug: 'middle_east__north_africa', :location_type => :country)
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'palestine').limit(1)[0]
o.locations << Location.where(slug: 'syria').limit(1)[0]
o.locations << Location.where(slug: 'albania').limit(1)[0]
o.locations << Location.where(slug: 'belarus').limit(1)[0]
o.locations << Location.create(name: "Bosnia &Herz.", slug: 'bosnia_herz', :location_type => :country)
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'kosovo').limit(1)[0]
o.locations << Location.where(slug: 'moldova').limit(1)[0]
o.locations << Location.where(slug: 'russia').limit(1)[0]
o.locations << Location.where(slug: 'serbia').limit(1)[0]
o.locations << Location.where(slug: 'turkey').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'bolivia').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'cuba').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o = Organization.create(name: "SweetSense, Inc.", slug: "sweetsense_inc", when_endorsed: DateTime.new(2017, 1, 1), is_endorser: true, website: "www.sweetsensors.com")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'water').limit(1)[0]
o.sectors << Sector.where(slug: 'energy').limit(1)[0]
o.sectors << Sector.where(slug: 'sanitation').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'rwanda').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'us').limit(1)[0]
o = Organization.create(name: "TechChange", slug: "techchange", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://www.techchange.org/")

o.sectors << Sector.where(slug: 'education__training').limit(1)[0]
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o = Organization.create(name: "TecSalud", slug: "tecsalud", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "https://www.tecsalud.io/")

o.locations << Location.where(slug: 'belize').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o = Organization.create(name: "Terre des Hommes", slug: "terre_des_hommes", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.ieda-project.org ")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
l = Location.create(name: "Lausanne, 15 Avenue Montchoisi Lausanne 1006, Switzerland", slug: 'lausanne__avenue_montchoisi_laus', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (46.5134439, 6.6338339) ] WHERE slug = 'lausanne__avenue_montchoisi_laus'")
o.locations << l
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.create(name: "Albanie", slug: 'albanie', :location_type => :country)
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.create(name: "Bénin", slug: 'bnin', :location_type => :country)
o.locations << Location.create(name: "Brésil", slug: 'brsil', :location_type => :country)
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.create(name: "Colombie", slug: 'colombie', :location_type => :country)
o.locations << Location.create(name: "Egypte", slug: 'egypte', :location_type => :country)
o.locations << Location.create(name: "Equateur", slug: 'equateur', :location_type => :country)
o.locations << Location.create(name: "Grèce", slug: 'grce', :location_type => :country)
o.locations << Location.create(name: "Guinée", slug: 'guine', :location_type => :country)
o.locations << Location.create(name: "Haïti", slug: 'hati', :location_type => :country)
o.locations << Location.create(name: "Hongrie", slug: 'hongrie', :location_type => :country)
o.locations << Location.create(name: "Inde", slug: 'inde', :location_type => :country)
o.locations << Location.create(name: "Irak", slug: 'irak', :location_type => :country)
o.locations << Location.create(name: "Jordanie", slug: 'jordanie', :location_type => :country)
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'kosovo').limit(1)[0]
o.locations << Location.create(name: "Liban", slug: 'liban', :location_type => :country)
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.create(name: "Maroc", slug: 'maroc', :location_type => :country)
o.locations << Location.create(name: "Mauritanie", slug: 'mauritanie', :location_type => :country)
o.locations << Location.create(name: "Moldavie", slug: 'moldavie', :location_type => :country)
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.create(name: "Népal", slug: 'npal', :location_type => :country)
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'palestine').limit(1)[0]
o.locations << Location.where(slug: 'panama').limit(1)[0]
o.locations << Location.create(name: "Pérou", slug: 'prou', :location_type => :country)
o.locations << Location.create(name: "Roumanie", slug: 'roumanie', :location_type => :country)
o.locations << Location.create(name: "Sénégal", slug: 'sngal', :location_type => :country)
o.locations << Location.create(name: "Soudan du Sud", slug: 'soudan_du_sud', :location_type => :country)
o.locations << Location.create(name: "Suisse", slug: 'suisse', :location_type => :country)
o.locations << Location.create(name: "Syrie", slug: 'syrie', :location_type => :country)
o.locations << Location.where(slug: 'togo').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o = Organization.create(name: "Tetra Tech", slug: "tetra_tech", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.tetratech.com/")

o.sectors << Sector.where(slug: 'water').limit(1)[0]
o.sectors << Sector.where(slug: 'environment').limit(1)[0]
o.sectors << Sector.where(slug: 'infrastructure').limit(1)[0]
o.sectors << Sector.where(slug: 'resource_marangement').limit(1)[0]
o.sectors << Sector.where(slug: 'energy').limit(1)[0]
o.locations << Location.create(name: "Africa (No Specific countries listed)", slug: 'africa_no_specific_countries_lis', :location_type => :country)
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'korea').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'austrailia').limit(1)[0]
o.locations << Location.where(slug: 'new_zealand').limit(1)[0]
o.locations << Location.where(slug: 'canada').limit(1)[0]
o.locations << Location.where(slug: 'france').limit(1)[0]
o.locations << Location.where(slug: 'germany').limit(1)[0]
o.locations << Location.where(slug: 'uk').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.where(slug: 'chile').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'uae').limit(1)[0]
o = Organization.create(name: "The Swedish Program for ICT in Developing Regions (SPIDER)", slug: "the_swedish_program_for_ict_in_d", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://spidercenter.org/")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'transparency__accountability').limit(1)[0]
o.sectors << Sector.where(slug: 'ictd').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.create(name: "East Africa", slug: 'east_africa', :location_type => :country)
o = Organization.create(name: "Tropical Health and Education Trust (THET)", slug: "tropical_health_and_education_tr", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.thet.org")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.locations << Location.where(slug: 'london_uk').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o = Organization.create(name: "ThinkMD", slug: "thinkmd", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.thinkmd.org/")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
l = Location.create(name: "Burlington, VA, USA", slug: 'burlington_va_usa', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (44.4758825, -73.21207199999999) ] WHERE slug = 'burlington_va_usa'")
o.locations << l
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'united_states').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o = Organization.create(name: "TolaData", slug: "toladata", when_endorsed: DateTime.new(2017, 1, 1), is_endorser: true, website: "http://toladata.com/")

o.sectors << Sector.where(slug: 'software').limit(1)[0]
l = Location.create(name: "Berlin, Germany", slug: 'berlin_germany', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (52.52000659999999, 13.404954) ] WHERE slug = 'berlin_germany'")
o.locations << l
o.locations << Location.create(name: "Active globally", slug: 'active_globally', :location_type => :country)
o = Organization.create(name: "UK Department for International Development (DFID)", slug: "uk_department_for_international", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "https://www.gov.uk/government/organisations/department-for-international-development")

o.locations << Location.create(name: "DOES NOT HAVE A REAL SITE", slug: 'does_not_have_a_real_site', :location_type => :country)
o = Organization.create(name: "United Nations Children’s Fund (UNICEF)", slug: "united_nations_childrens_fund_un", when_endorsed: DateTime.new(2015, 1, 1), is_endorser: true, website: "https://www.unicef.org/")

o.sectors << Sector.where(slug: 'youth').limit(1)[0]
o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'gender_equality').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'albania').limit(1)[0]
o.locations << Location.where(slug: 'algeria').limit(1)[0]
o.locations << Location.where(slug: 'andorra').limit(1)[0]
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.where(slug: 'anguilla').limit(1)[0]
o.locations << Location.where(slug: 'antigua_and_barbuda').limit(1)[0]
o.locations << Location.where(slug: 'argentina').limit(1)[0]
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.where(slug: 'australia').limit(1)[0]
o.locations << Location.where(slug: 'austria').limit(1)[0]
o.locations << Location.where(slug: 'azerbaijan').limit(1)[0]
o.locations << Location.where(slug: 'bahrain').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'barbados').limit(1)[0]
o.locations << Location.where(slug: 'belarus').limit(1)[0]
o.locations << Location.where(slug: 'belgium').limit(1)[0]
o.locations << Location.where(slug: 'belize').limit(1)[0]
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'bhutan').limit(1)[0]
o.locations << Location.where(slug: 'bolivia').limit(1)[0]
o.locations << Location.create(name: "Plurinational State of Bosnia and Herzegovina", slug: 'plurinational_state_of_bosnia_an', :location_type => :country)
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.where(slug: 'british_virgin_islands').limit(1)[0]
o.locations << Location.where(slug: 'bulgaria').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'cabo_verde').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.where(slug: 'canada').limit(1)[0]
o.locations << Location.where(slug: 'central_african_republic').limit(1)[0]
o.locations << Location.where(slug: 'chad').limit(1)[0]
o.locations << Location.where(slug: 'chile').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'comoros').limit(1)[0]
o.locations << Location.where(slug: 'congo').limit(1)[0]
o.locations << Location.create(name: "Cook Islands", slug: 'cook_islands', :location_type => :country)
o.locations << Location.where(slug: 'costa_rica').limit(1)[0]
o.locations << Location.where(slug: 'cte_divoire').limit(1)[0]
o.locations << Location.where(slug: 'croatia').limit(1)[0]
o.locations << Location.where(slug: 'cuba').limit(1)[0]
o.locations << Location.where(slug: 'czech_republic').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'denmark').limit(1)[0]
o.locations << Location.where(slug: 'djibouti').limit(1)[0]
o.locations << Location.create(name: "Dominica", slug: 'dominica', :location_type => :country)
o.locations << Location.where(slug: 'dominican_republic').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'equatorial_guinea').limit(1)[0]
o.locations << Location.create(name: "Eritre", slug: 'eritre', :location_type => :country)
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'fiji').limit(1)[0]
o.locations << Location.where(slug: 'finland').limit(1)[0]
o.locations << Location.where(slug: 'france').limit(1)[0]
o.locations << Location.where(slug: 'gabon').limit(1)[0]
o.locations << Location.where(slug: 'gambia').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'germany').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'greece').limit(1)[0]
o.locations << Location.where(slug: 'grenada').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'guineabissau').limit(1)[0]
o.locations << Location.where(slug: 'guyana').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.create(name: "Hong Kong (Special Administrative Region", slug: 'hong_kong_special_administrative', :location_type => :country)
o.locations << Location.create(name: "People’s Republic)", slug: 'peoples_republic', :location_type => :country)
o.locations << Location.where(slug: 'hungary').limit(1)[0]
o.locations << Location.create(name: "Iceland", slug: 'iceland', :location_type => :country)
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'iran').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'ireland').limit(1)[0]
o.locations << Location.where(slug: 'israel').limit(1)[0]
o.locations << Location.where(slug: 'italy').limit(1)[0]
o.locations << Location.where(slug: 'jamaica').limit(1)[0]
o.locations << Location.where(slug: 'japan').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.create(name: "Democratic People's Republic of Korea (North Korea)", slug: 'democratic_peoples_republic_of_k', :location_type => :country)
o.locations << Location.where(slug: 'kazakhstan').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.create(name: "Kiribati", slug: 'kiribati', :location_type => :country)
o.locations << Location.create(name: "Kosovo (under UNSC Resolution 1244)", slug: 'kosovo_under_unsc_resolution_', :location_type => :country)
o.locations << Location.where(slug: 'kuwait').limit(1)[0]
o.locations << Location.where(slug: 'kyrgyzstan').limit(1)[0]
o.locations << Location.create(name: "Republic of Korea", slug: 'republic_of_korea', :location_type => :country)
o.locations << Location.create(name: "Lao People's Democratic Republic", slug: 'lao_peoples_democratic_republic', :location_type => :country)
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'lesotho').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'libya').limit(1)[0]
o.locations << Location.where(slug: 'lithuania').limit(1)[0]
o.locations << Location.create(name: "Luxembourg", slug: 'luxembourg', :location_type => :country)
o.locations << Location.where(slug: 'macedonia').limit(1)[0]
o.locations << Location.create(name: "the former Yugoslav Republic of", slug: 'the_former_yugoslav_republic_of', :location_type => :country)
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'malaysia').limit(1)[0]
o.locations << Location.where(slug: 'maldives').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'marshall_islands').limit(1)[0]
o.locations << Location.where(slug: 'mauritania').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.create(name: "Micronesia (Federated States of)", slug: 'micronesia_federated_states_of', :location_type => :country)
o.locations << Location.where(slug: 'moldova').limit(1)[0]
o.locations << Location.where(slug: 'mongolia').limit(1)[0]
o.locations << Location.where(slug: 'montenegro').limit(1)[0]
o.locations << Location.where(slug: 'montserrat').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'namibia').limit(1)[0]
o.locations << Location.where(slug: 'nauru').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'netherlands').limit(1)[0]
o.locations << Location.where(slug: 'new_zealand').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.create(name: "Niue", slug: 'niue', :location_type => :country)
o.locations << Location.where(slug: 'norway').limit(1)[0]
o.locations << Location.where(slug: 'oman').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.create(name: "Palau", slug: 'palau', :location_type => :country)
o.locations << Location.where(slug: 'panama').limit(1)[0]
o.locations << Location.where(slug: 'papua_new_guinea').limit(1)[0]
o.locations << Location.where(slug: 'paraguay').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'poland').limit(1)[0]
o.locations << Location.where(slug: 'portugal').limit(1)[0]
o.locations << Location.where(slug: 'qatar').limit(1)[0]
o.locations << Location.where(slug: 'romania').limit(1)[0]
o.locations << Location.where(slug: 'rwanda').limit(1)[0]
o.locations << Location.where(slug: 'saint_kitts_and_nevis').limit(1)[0]
o.locations << Location.where(slug: 'saint_lucia').limit(1)[0]
o.locations << Location.where(slug: 'saint_vincent_and_the_grenadines').limit(1)[0]
o.locations << Location.where(slug: 'samoa').limit(1)[0]
o.locations << Location.where(slug: 'sao_tome_and_principe').limit(1)[0]
o.locations << Location.where(slug: 'saudi_arabia').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'serbia').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'slovakia').limit(1)[0]
o.locations << Location.where(slug: 'slovenia').limit(1)[0]
o.locations << Location.where(slug: 'solomon_islands').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'spain').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.create(name: "State of Palestine", slug: 'state_of_palestine', :location_type => :country)
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 'suriname').limit(1)[0]
o.locations << Location.where(slug: 'swaziland').limit(1)[0]
o.locations << Location.where(slug: 'sweden').limit(1)[0]
o.locations << Location.where(slug: 'switzerland').limit(1)[0]
o.locations << Location.where(slug: 'syrian_arab_republic').limit(1)[0]
o.locations << Location.where(slug: 'tajikistan').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'timorleste').limit(1)[0]
o.locations << Location.where(slug: 'togo').limit(1)[0]
o.locations << Location.create(name: "Tokelau", slug: 'tokelau', :location_type => :country)
o.locations << Location.where(slug: 'tonga').limit(1)[0]
o.locations << Location.where(slug: 'trinidad_and_tobago').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'turkey').limit(1)[0]
o.locations << Location.where(slug: 'turkmenistan').limit(1)[0]
o.locations << Location.create(name: "Turks and Caicos Islands", slug: 'turks_and_caicos_islands', :location_type => :country)
o.locations << Location.create(name: "Tuvalu", slug: 'tuvalu', :location_type => :country)
o.locations << Location.create(name: "United Republic of Tanzania", slug: 'united_republic_of_tanzania', :location_type => :country)
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'united_arab_emirates').limit(1)[0]
o.locations << Location.create(name: "United Kingdom of Great Britain and Northern Ireland", slug: 'united_kingdom_of_great_britain', :location_type => :country)
o.locations << Location.where(slug: 'united_states_of_america').limit(1)[0]
o.locations << Location.where(slug: 'uruguay').limit(1)[0]
o.locations << Location.where(slug: 'uzbekistan').limit(1)[0]
o.locations << Location.where(slug: 'vanuatu').limit(1)[0]
o.locations << Location.where(slug: 'venezuela').limit(1)[0]
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o = Organization.create(name: "UN Climate Change Secretariat", slug: "un_climate_change_secretariat", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "unfccc.int")

o.sectors << Sector.where(slug: 'environment__cllimate_change').limit(1)[0]
o.locations << Location.create(name: "Not Listed", slug: 'not_listed', :location_type => :country)
o = Organization.create(name: "United Nations Development Programme (UNDP)", slug: "united_nations_development_progr", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.undp.org/en")

o.sectors << Sector.where(slug: 'gender_equality').limit(1)[0]
o.sectors << Sector.where(slug: 'crisis_response').limit(1)[0]
o.sectors << Sector.where(slug: 'development_impact').limit(1)[0]
o.locations << Location.create(name: "170 Countries", slug: '_countries', :location_type => :country)
o.locations << Location.create(name: "not specified", slug: 'not_specified', :location_type => :country)
o = Organization.create(name: "United Nations Foundation (UNF)", slug: "united_nations_foundation_unf", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.unfoundation.org")

o.sectors << Sector.where(slug: 'energy__climate').limit(1)[0]
o.sectors << Sector.where(slug: 'global_health').limit(1)[0]
l = Location.create(name: "DC", slug: 'dc', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (37.09024, -95.712891) ] WHERE slug = 'dc'")
o.locations << l
o = Organization.create(name: "United Nations Global Pulse", slug: "united_nations_global_pulse", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.unglobalpulse.org/")

o.sectors << Sector.where(slug: 'big_data').limit(1)[0]
o = Organization.create(name: "United Nations Office for the Coordination of Humanitarian Affairs (OCHA)", slug: "united_nations_office_for_the_co", when_endorsed: DateTime.new(2015, 1, 1), is_endorser: true, website: "https://www.unocha.org/")

o.sectors << Sector.where(slug: 'humaitarian').limit(1)[0]
o.sectors << Sector.where(slug: 'emergency').limit(1)[0]
l = Location.create(name: "NYC", slug: 'nyc', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (40.7127753, -74.0059728) ] WHERE slug = 'nyc'")
o.locations << l
l = Location.create(name: "Geneva, Switzerland", slug: 'geneva_switzerland', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (46.2043907, 6.1431577) ] WHERE slug = 'geneva_switzerland'")
o.locations << l
o.locations << Location.where(slug: 'panama').limit(1)[0]
l = Location.create(name: "Bangkok, Thailand", slug: 'bangkok_thailand', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (13.7563309, 100.5017651) ] WHERE slug = 'bangkok_thailand'")
o.locations << l
l = Location.create(name: "Dhakar, Senegal", slug: 'dhakar_senegal', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (14.716677, -17.4676861) ] WHERE slug = 'dhakar_senegal'")
o.locations << l
o.locations << Location.where(slug: 'nairobi_kenya').limit(1)[0]
l = Location.create(name: "Office of Pacific Islands (near New Zealand)", slug: 'office_of_pacific_islands_near_n', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (-41.282324, 174.774992) ] WHERE slug = 'office_of_pacific_islands_near_n'")
o.locations << l
o.locations << Location.where(slug: 'syria').limit(1)[0]
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'hondura').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'venezuela').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'bolivia').limit(1)[0]
o.locations << Location.where(slug: 'switzerland').limit(1)[0]
o.locations << Location.where(slug: 'panama').limit(1)[0]
o.locations << Location.where(slug: 'libya').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'chad').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.create(name: "CAR", slug: 'car', :location_type => :country)
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.where(slug: 'drc').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.create(name: "Eriteria", slug: 'eriteria', :location_type => :country)
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'kazakhstan').limit(1)[0]
o.locations << Location.create(name: "Kyrgzystan", slug: 'kyrgzystan', :location_type => :country)
o.locations << Location.where(slug: 'tajikistan').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'turkey').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'uae').limit(1)[0]
o.locations << Location.where(slug: 'iran').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'north_korea').limit(1)[0]
o.locations << Location.where(slug: 'japan').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o = Organization.create(name: "United Nations Population Fund (UNFPA)", slug: "united_nations_population_fund_u", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.unfpa.org")

o.sectors << Sector.where(slug: 'adolescent_and_youth_female_gent').limit(1)[0]
o.locations << Location.where(slug: 'us').limit(1)[0]
o.locations << Location.where(slug: 'belgium').limit(1)[0]
l = Location.create(name: "Denamrk", slug: 'denamrk', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (56.26392, 9.501785) ] WHERE slug = 'denamrk'")
o.locations << l
o.locations << Location.where(slug: 'switzerland').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'japan').limit(1)[0]
o.locations << Location.where(slug: 'us').limit(1)[0]
o.locations << Location.where(slug: 'belgium').limit(1)[0]
o.locations << Location.where(slug: 'denamrk').limit(1)[0]
o.locations << Location.where(slug: 'switzerland').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'japan').limit(1)[0]
o.locations << Location.where(slug: 'jamaica').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'panama').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'lesotho').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.create(name: "Malawai", slug: 'malawai', :location_type => :country)
o.locations << Location.create(name: "The Comoros", slug: 'the_comoros', :location_type => :country)
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'chad').limit(1)[0]
o.locations << Location.where(slug: 'drc').limit(1)[0]
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'sengal').limit(1)[0]
o.locations << Location.where(slug: 'guineabissau').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'bosnia__herz').limit(1)[0]
o.locations << Location.where(slug: 'moldova').limit(1)[0]
o.locations << Location.where(slug: 'turkey').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'azerbaijan').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'syria').limit(1)[0]
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'oman').limit(1)[0]
o.locations << Location.create(name: "Aghanistan", slug: 'aghanistan', :location_type => :country)
o.locations << Location.where(slug: 'maldives').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'laos').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'papua_new_guinea').limit(1)[0]
o.locations << Location.where(slug: 'japan').limit(1)[0]
o.locations << Location.create(name: "Figi", slug: 'figi', :location_type => :country)
o = Organization.create(name: "United States Agency for International Development (USAID)", slug: "united_states_agency_for_interna", when_endorsed: DateTime.new(2015, 1, 1), is_endorser: true, website: "https://www.usaid.gov/")

o.sectors << Sector.where(slug: 'agriculture_and_food_security').limit(1)[0]
o.sectors << Sector.where(slug: 'democracy').limit(1)[0]
o.sectors << Sector.where(slug: 'human_rights_and_governance').limit(1)[0]
o.sectors << Sector.where(slug: 'economic_growth_and_trade').limit(1)[0]
o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'environment_and_global_climate_c').limit(1)[0]
o.sectors << Sector.where(slug: 'gender_equality_and_womens_empow').limit(1)[0]
o.sectors << Sector.where(slug: 'global_health').limit(1)[0]
o.sectors << Sector.where(slug: 'water_and_sanitation').limit(1)[0]
o.sectors << Sector.where(slug: 'working_in_crises_and_conflict').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'albania').limit(1)[0]
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.where(slug: 'azerbaijan').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'belarus').limit(1)[0]
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'bosnia_and_herzegovina').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'burma').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.where(slug: 'chad').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'cte_divoire').limit(1)[0]
o.locations << Location.where(slug: 'cuba').limit(1)[0]
o.locations << Location.where(slug: 'cyprus').limit(1)[0]
o.locations << Location.where(slug: 'dr_congo').limit(1)[0]
o.locations << Location.where(slug: 'djibouti').limit(1)[0]
o.locations << Location.where(slug: 'dominican_republic').limit(1)[0]
o.locations << Location.create(name: "Eastern and Southern Caribbean", slug: 'eastern_and_southern_caribbean', :location_type => :country)
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'eswatini').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'guyana').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'jamaica').limit(1)[0]
o.locations << Location.where(slug: 'kazakhstan').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'kosovo').limit(1)[0]
o.locations << Location.where(slug: 'kyrgyz_republic').limit(1)[0]
o.locations << Location.where(slug: 'laos').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'lesotho').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'libya').limit(1)[0]
o.locations << Location.where(slug: 'macedonia').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'maldives').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'mauritania').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'moldova').limit(1)[0]
o.locations << Location.where(slug: 'mongolia').limit(1)[0]
o.locations << Location.where(slug: 'montenegro').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'namibia').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.create(name: "Pacific Islands", slug: 'pacific_islands', :location_type => :country)
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'panama').limit(1)[0]
o.locations << Location.where(slug: 'paraguay').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'republic_of_the_congo').limit(1)[0]
o.locations << Location.where(slug: 'rwanda').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'serbia').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 'syria').limit(1)[0]
o.locations << Location.where(slug: 'tajikistan').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'timorleste').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'turkmenistan').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'uzbekistan').limit(1)[0]
o.locations << Location.where(slug: 'venezuela').limit(1)[0]
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o.locations << Location.where(slug: 'west_bank_and_gaza').limit(1)[0]
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o.locations << Location.create(name: " | Jordan", slug: '__jordan', :location_type => :country)
o.locations << Location.create(name: " | Egypt", slug: '__egypt', :location_type => :country)
o = Organization.create(name: "VillageReach", slug: "villagereach", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.villagereach.org/")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
l = Location.create(name: "Seattle, WA, USA", slug: 'seattle_wa_usa', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (47.6062095, -122.3320708) ] WHERE slug = 'seattle_wa_usa'")
o.locations << l
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'cote_divoire').limit(1)[0]
o.locations << Location.where(slug: 'drc').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'togo').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o = Organization.create(name: "VitalWave", slug: "vitalwave", when_endorsed: DateTime.new(2017, 1, 1), is_endorser: true, website: "www.vitalwave.com")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
o.sectors << Sector.where(slug: 'finacial_services').limit(1)[0]
o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'agriculture').limit(1)[0]
o.sectors << Sector.where(slug: 'gender').limit(1)[0]
o.sectors << Sector.where(slug: 'economic').limit(1)[0]
o.sectors << Sector.where(slug: 'energy').limit(1)[0]
o.sectors << Sector.where(slug: 'ictd').limit(1)[0]
l = Location.create(name: "Paolo Alto, CA, USA", slug: 'paolo_alto_ca_usa', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (37.4418834, -122.1430195) ] WHERE slug = 'paolo_alto_ca_usa'")
o.locations << l
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'usa').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'uk').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'albania').limit(1)[0]
o.locations << Location.where(slug: 'portugal').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'montenegro').limit(1)[0]
o.locations << Location.where(slug: 'mongolia').limit(1)[0]
o.locations << Location.where(slug: 'africa').limit(1)[0]
o.locations << Location.where(slug: 'asia').limit(1)[0]
o.locations << Location.where(slug: 'europe').limit(1)[0]
o.locations << Location.create(name: "Latin America: All countries were not listed)", slug: 'latin_america_all_countries_were', :location_type => :country)
o = Organization.create(name: "VOTO Mobile", slug: "voto_mobile", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "https://www.votomobile.org/")

o.sectors << Sector.where(slug: 'ictd').limit(1)[0]
l = Location.create(name: "Current Offices:", slug: 'current_offices', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (36.8506952, -95.63566569999999) ] WHERE slug = 'current_offices'")
o.locations << l
l = Location.create(name: "Canada, US, UK, Senegal, Mali, Burkina Faso, Ghana, Niger, Nigeria, Ethiopia, Kenya, Uganda, Rawanda, DRC, Tanzania, Zambia, Malwai, Mozambique, Zimbabwe, Botswana, South Africa, Madagascar, Afghanistan, Pakistan, Nepal, India, Cambodia, Indonesia,", slug: 'canada_us_uk_senegal_mali_burkin', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (56.130366, -106.346771) ] WHERE slug = 'canada_us_uk_senegal_mali_burkin'")
o.locations << l
o.locations << Location.create(name: "All Countries", slug: 'all_countries', :location_type => :country)
o = Organization.create(name: "Welthungerhilfe", slug: "welthungerhilfe", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "www.welthungerhilfe.de")

o.sectors << Sector.where(slug: 'agriculture').limit(1)[0]
o.sectors << Sector.where(slug: 'environment').limit(1)[0]
o.sectors << Sector.where(slug: 'nutrition').limit(1)[0]
o.sectors << Sector.where(slug: 'water').limit(1)[0]
o.sectors << Sector.where(slug: 'sanitation').limit(1)[0]
o.sectors << Sector.where(slug: 'economic').limit(1)[0]
o.sectors << Sector.where(slug: 'advocacy').limit(1)[0]
o.sectors << Sector.where(slug: 'refugees').limit(1)[0]
l = Location.create(name: "Germany (maybe)", slug: 'germany_maybe', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (51.165691, 10.451526) ] WHERE slug = 'germany_maybe'")
o.locations << l
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'bolivia').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'car').limit(1)[0]
o.locations << Location.where(slug: 'drc').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'somaliland').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.create(name: "Syria/Turkey", slug: 'syriaturkey', :location_type => :country)
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'laos').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'north_korea').limit(1)[0]
o.locations << Location.where(slug: 'tajikistan').limit(1)[0]
o = Organization.create(name: "World Agroforestry Center (ICRAF)", slug: "world_agroforestry_center_icraf", when_endorsed: DateTime.new(2018, 1, 1), is_endorser: true, website: "http://worldagroforestry.org/")

o.sectors << Sector.where(slug: 'environment').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.where(slug: 'ivory_coast').limit(1)[0]
o.locations << Location.where(slug: 'drc').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'costa_rica').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'bhutan').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'maldives').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.create(name: "Philipphines", slug: 'philipphines', :location_type => :country)
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o = Organization.create(name: "World Bank", slug: "world_bank", when_endorsed: DateTime.new(2016, 1, 1), is_endorser: true, website: "http://www.worldbank.org/")

o.sectors << Sector.where(slug: 'finance').limit(1)[0]
o.sectors << Sector.where(slug: 'gender').limit(1)[0]
o.sectors << Sector.where(slug: 'governance').limit(1)[0]
o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'water').limit(1)[0]
o.sectors << Sector.where(slug: 'agriculture').limit(1)[0]
o.sectors << Sector.where(slug: 'energy').limit(1)[0]
o.sectors << Sector.where(slug: 'environment').limit(1)[0]
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'albania').limit(1)[0]
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.create(name: "Antigua and Barbuda Argentina", slug: 'antigua_and_barbuda_argentina', :location_type => :country)
o.locations << Location.where(slug: 'algeria').limit(1)[0]
o.locations << Location.where(slug: 'austria').limit(1)[0]
o.locations << Location.where(slug: 'azerbaijan').limit(1)[0]
o.locations << Location.where(slug: 'bahrain').limit(1)[0]
o.locations << Location.where(slug: 'belgium').limit(1)[0]
o.locations << Location.where(slug: 'bhutan').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'belarus').limit(1)[0]
o.locations << Location.where(slug: 'belize').limit(1)[0]
o.locations << Location.where(slug: 'bolivia').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'bosnia_and_herzegovina').limit(1)[0]
o.locations << Location.where(slug: 'bulgaria').limit(1)[0]
o.locations << Location.where(slug: 'cabo_verde').limit(1)[0]
o.locations << Location.where(slug: 'canada').limit(1)[0]
o.locations << Location.where(slug: 'chile').limit(1)[0]
o.locations << Location.where(slug: 'comoros').limit(1)[0]
o.locations << Location.where(slug: 'costa_rica').limit(1)[0]
o.locations << Location.where(slug: 'czech_republic').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'car').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'drc').limit(1)[0]
o.locations << Location.where(slug: 'ivory_coast').limit(1)[0]
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.where(slug: 'chad').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.create(name: "Republic of Congo", slug: 'republic_of_congo', :location_type => :country)
o.locations << Location.where(slug: 'croatia').limit(1)[0]
o.locations << Location.where(slug: 'denmark').limit(1)[0]
o.locations << Location.where(slug: 'djibouti').limit(1)[0]
o.locations << Location.where(slug: 'dominica').limit(1)[0]
o.locations << Location.where(slug: 'dominican_republic').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.create(name: "Equatorial", slug: 'equatorial', :location_type => :country)
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'eritrea').limit(1)[0]
o.locations << Location.where(slug: 'estonia').limit(1)[0]
o.locations << Location.where(slug: 'eswatini').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'fiji').limit(1)[0]
o.locations << Location.where(slug: 'finland').limit(1)[0]
o.locations << Location.where(slug: 'france').limit(1)[0]
o.locations << Location.where(slug: 'gabon').limit(1)[0]
o.locations << Location.where(slug: 'the_gambia').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'germany').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'greece').limit(1)[0]
o.locations << Location.where(slug: 'grenada').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.create(name: "Guinea-Bassu", slug: 'guineabassu', :location_type => :country)
o.locations << Location.where(slug: 'guyana').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'hungary').limit(1)[0]
o.locations << Location.where(slug: 'iceland').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'iran').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'israel').limit(1)[0]
o.locations << Location.where(slug: 'italy').limit(1)[0]
o.locations << Location.where(slug: 'jamaica').limit(1)[0]
o.locations << Location.where(slug: 'japan').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'kazakhstan').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'kiribati').limit(1)[0]
o.locations << Location.where(slug: 'korea').limit(1)[0]
o.locations << Location.where(slug: 'kosovo').limit(1)[0]
o.locations << Location.create(name: "Kosovo. Kyrgz Republic", slug: 'kosovo_kyrgz_republic', :location_type => :country)
o.locations << Location.create(name: "Lao PDR", slug: 'lao_pdr', :location_type => :country)
o.locations << Location.where(slug: 'latvia').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'lesotho').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'libya').limit(1)[0]
o.locations << Location.where(slug: 'lithuania').limit(1)[0]
o.locations << Location.where(slug: 'luxembourg').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'malaysia').limit(1)[0]
o.locations << Location.where(slug: 'maldives').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'marshall_islands').limit(1)[0]
o.locations << Location.where(slug: 'mauritania').limit(1)[0]
o.locations << Location.where(slug: 'mauritius').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.create(name: "FS Micronesia", slug: 'fs_micronesia', :location_type => :country)
o.locations << Location.where(slug: 'moldova').limit(1)[0]
o.locations << Location.where(slug: 'mongolia').limit(1)[0]
o.locations << Location.where(slug: 'montenegro').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'namibia').limit(1)[0]
o.locations << Location.where(slug: 'nauru').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'netherlands').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'norway').limit(1)[0]
o.locations << Location.where(slug: 'oman').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'palau').limit(1)[0]
o.locations << Location.where(slug: 'panama').limit(1)[0]
o.locations << Location.where(slug: 'papua').limit(1)[0]
o.locations << Location.where(slug: 'new_guinea').limit(1)[0]
o.locations << Location.where(slug: 'paraguay').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'poland').limit(1)[0]
o.locations << Location.where(slug: 'portugal').limit(1)[0]
o.locations << Location.where(slug: 'romania').limit(1)[0]
o.locations << Location.where(slug: 'russia').limit(1)[0]
o.locations << Location.where(slug: 'rawanda').limit(1)[0]
o.locations << Location.where(slug: 'samoa').limit(1)[0]
o.locations << Location.where(slug: 'sao_tome_and_principe').limit(1)[0]
o.locations << Location.where(slug: 'saudi_arabia').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'serbia').limit(1)[0]
o.locations << Location.where(slug: 'seychelles').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'singapore').limit(1)[0]
o.locations << Location.where(slug: 'slovak_republic').limit(1)[0]
o.locations << Location.where(slug: 'slovenia').limit(1)[0]
o.locations << Location.where(slug: 'solomon_islands').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'spain').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.create(name: "St. Kitts and Nevis", slug: 'st_kitts_and_nevis', :location_type => :country)
o.locations << Location.create(name: "St. Lucia", slug: 'st_lucia', :location_type => :country)
o.locations << Location.create(name: "St. Vincent and Grenadines", slug: 'st_vincent_and_grenadines', :location_type => :country)
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 'suriname').limit(1)[0]
o.locations << Location.where(slug: 'swaziland').limit(1)[0]
o.locations << Location.where(slug: 'sweden').limit(1)[0]
o.locations << Location.where(slug: 'switzerland').limit(1)[0]
o.locations << Location.where(slug: 'syria').limit(1)[0]
o.locations << Location.where(slug: 'tajikistan').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.create(name: "Timor Leste (East Timor)", slug: 'timor_leste_east_timor', :location_type => :country)
o.locations << Location.where(slug: 'togo').limit(1)[0]
o.locations << Location.where(slug: 'tonga').limit(1)[0]
o.locations << Location.where(slug: 'trinidad_and_tobago').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'turkey').limit(1)[0]
o.locations << Location.where(slug: 'turkmenistan').limit(1)[0]
o.locations << Location.where(slug: 'tuvalu').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'uae').limit(1)[0]
o.locations << Location.where(slug: 'united_kingdom').limit(1)[0]
o.locations << Location.where(slug: 'united_states_of_america').limit(1)[0]
o.locations << Location.where(slug: 'uruguay').limit(1)[0]
o.locations << Location.where(slug: 'uzbekistan').limit(1)[0]
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o = Organization.create(name: "World Food Programme (WFP)", slug: "world_food_programme_wfp", when_endorsed: DateTime.new(2015, 1, 1), is_endorser: true, website: "http://www1.wfp.org/")

o.sectors << Sector.where(slug: 'emergency').limit(1)[0]
o.sectors << Sector.where(slug: 'food_assistance').limit(1)[0]
l = Location.create(name: "Rome, Italy", slug: 'rome_italy', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (41.9027835, 12.4963655) ] WHERE slug = 'rome_italy'")
o.locations << l
o.locations << Location.where(slug: 'washington_dc').limit(1)[0]
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'algeria').limit(1)[0]
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'bhutan').limit(1)[0]
o.locations << Location.create(name: "Bolivia (Plurinational State of)", slug: 'bolivia_plurinational_state_of', :location_type => :country)
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.where(slug: 'car').limit(1)[0]
o.locations << Location.where(slug: 'chad').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'congo').limit(1)[0]
o.locations << Location.where(slug: 'cuba').limit(1)[0]
o.locations << Location.where(slug: 'cte_divoire').limit(1)[0]
o.locations << Location.where(slug: 'democratic_peoples_republic_of_k').limit(1)[0]
o.locations << Location.where(slug: 'drc').limit(1)[0]
o.locations << Location.where(slug: 'djibouti').limit(1)[0]
o.locations << Location.where(slug: 'dominican_republic').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'eswatini').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'gambia').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'guineabissau').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.create(name: "Iran (Islamic Republic of)", slug: 'iran_islamic_republic_of', :location_type => :country)
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'kyrgyzstan').limit(1)[0]
o.locations << Location.where(slug: 'lao_peoples_democratic_republic').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'lesotho').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'libya').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'mauritania').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'namibia').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'rwanda').limit(1)[0]
o.locations << Location.where(slug: 'sao_tome_and_principe').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'state_of_palestine').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 'syrian_arab_republic').limit(1)[0]
o.locations << Location.where(slug: 'tajikistan').limit(1)[0]
o.locations << Location.where(slug: 'tanzania').limit(1)[0]
o.locations << Location.create(name: "The Pacific", slug: 'the_pacific', :location_type => :country)
o.locations << Location.where(slug: 'timorleste').limit(1)[0]
o.locations << Location.where(slug: 'togo').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'turkey').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o = Organization.create(name: "World Health Organization (WHO)", slug: "world_health_organization_who", when_endorsed: DateTime.new(2015, 1, 1), is_endorser: true, website: "http://www.who.int/en/")

o.sectors << Sector.where(slug: 'health').limit(1)[0]
l = Location.create(name: "Geneva", slug: 'geneva', :location_type => :point)
connection.execute("UPDATE locations SET points = ARRAY[ POINT (46.2043907, 6.1431577) ] WHERE slug = 'geneva'")
o.locations << l
o.locations << Location.where(slug: 'afghanistan').limit(1)[0]
o.locations << Location.where(slug: 'albania').limit(1)[0]
o.locations << Location.where(slug: 'algeria').limit(1)[0]
o.locations << Location.where(slug: 'andorra').limit(1)[0]
o.locations << Location.where(slug: 'angola').limit(1)[0]
o.locations << Location.where(slug: 'antigua_and_barbuda').limit(1)[0]
o.locations << Location.where(slug: 'argentina').limit(1)[0]
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.where(slug: 'australia').limit(1)[0]
o.locations << Location.where(slug: 'austria').limit(1)[0]
o.locations << Location.where(slug: 'azerbaijan').limit(1)[0]
o.locations << Location.create(name: "Bahamas", slug: 'bahamas', :location_type => :country)
o.locations << Location.where(slug: 'bahrain').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'barbados').limit(1)[0]
o.locations << Location.where(slug: 'belarus').limit(1)[0]
o.locations << Location.where(slug: 'belgium').limit(1)[0]
o.locations << Location.where(slug: 'belize').limit(1)[0]
o.locations << Location.where(slug: 'benin').limit(1)[0]
o.locations << Location.where(slug: 'bhutan').limit(1)[0]
o.locations << Location.where(slug: 'bolivia_plurinational_state_of').limit(1)[0]
o.locations << Location.where(slug: 'bosnia_and_herzegovina').limit(1)[0]
o.locations << Location.where(slug: 'botswana').limit(1)[0]
o.locations << Location.where(slug: 'brazil').limit(1)[0]
o.locations << Location.create(name: "Brunei Darussalam", slug: 'brunei_darussalam', :location_type => :country)
o.locations << Location.where(slug: 'bulgaria').limit(1)[0]
o.locations << Location.where(slug: 'burkina_faso').limit(1)[0]
o.locations << Location.where(slug: 'burundi').limit(1)[0]
o.locations << Location.where(slug: 'cabo_verde').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'cameroon').limit(1)[0]
o.locations << Location.where(slug: 'canada').limit(1)[0]
o.locations << Location.where(slug: 'car').limit(1)[0]
o.locations << Location.where(slug: 'chad').limit(1)[0]
o.locations << Location.where(slug: 'chile').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'colombia').limit(1)[0]
o.locations << Location.where(slug: 'comoros').limit(1)[0]
o.locations << Location.where(slug: 'congo').limit(1)[0]
o.locations << Location.where(slug: 'cook_islands').limit(1)[0]
o.locations << Location.where(slug: 'costa_rica').limit(1)[0]
o.locations << Location.where(slug: 'cte_divoire').limit(1)[0]
o.locations << Location.where(slug: 'croatia').limit(1)[0]
o.locations << Location.where(slug: 'cuba').limit(1)[0]
o.locations << Location.where(slug: 'cyprus').limit(1)[0]
o.locations << Location.create(name: "Czechia", slug: 'czechia', :location_type => :country)
o.locations << Location.where(slug: 'democratic_peoples_republic_of_k').limit(1)[0]
o.locations << Location.where(slug: 'democratic_republic_of_the_congo').limit(1)[0]
o.locations << Location.where(slug: 'denmark').limit(1)[0]
o.locations << Location.where(slug: 'djibouti').limit(1)[0]
o.locations << Location.where(slug: 'dominica').limit(1)[0]
o.locations << Location.where(slug: 'dominican_republic').limit(1)[0]
o.locations << Location.where(slug: 'ecuador').limit(1)[0]
o.locations << Location.where(slug: 'egypt').limit(1)[0]
o.locations << Location.where(slug: 'el_salvador').limit(1)[0]
o.locations << Location.where(slug: 'equatorial_guinea').limit(1)[0]
o.locations << Location.where(slug: 'eritrea').limit(1)[0]
o.locations << Location.where(slug: 'estonia').limit(1)[0]
o.locations << Location.where(slug: 'eswatini').limit(1)[0]
o.locations << Location.where(slug: 'ethiopia').limit(1)[0]
o.locations << Location.where(slug: 'fiji').limit(1)[0]
o.locations << Location.where(slug: 'finland').limit(1)[0]
o.locations << Location.where(slug: 'france').limit(1)[0]
o.locations << Location.where(slug: 'gabon').limit(1)[0]
o.locations << Location.where(slug: 'gambia').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'germany').limit(1)[0]
o.locations << Location.where(slug: 'ghana').limit(1)[0]
o.locations << Location.where(slug: 'greece').limit(1)[0]
o.locations << Location.where(slug: 'grenada').limit(1)[0]
o.locations << Location.where(slug: 'guatemala').limit(1)[0]
o.locations << Location.where(slug: 'guinea').limit(1)[0]
o.locations << Location.where(slug: 'guineabissau').limit(1)[0]
o.locations << Location.where(slug: 'guyana').limit(1)[0]
o.locations << Location.where(slug: 'haiti').limit(1)[0]
o.locations << Location.where(slug: 'honduras').limit(1)[0]
o.locations << Location.where(slug: 'hungary').limit(1)[0]
o.locations << Location.where(slug: 'iceland').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'iran_islamic_republic_of').limit(1)[0]
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'ireland').limit(1)[0]
o.locations << Location.where(slug: 'israel').limit(1)[0]
o.locations << Location.where(slug: 'italy').limit(1)[0]
o.locations << Location.where(slug: 'jamaica').limit(1)[0]
o.locations << Location.where(slug: 'japan').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'kazakhstan').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]
o.locations << Location.where(slug: 'kiribati').limit(1)[0]
o.locations << Location.where(slug: 'kuwait').limit(1)[0]
o.locations << Location.where(slug: 'kyrgyzstan').limit(1)[0]
o.locations << Location.where(slug: 'lao_peoples_democratic_republic').limit(1)[0]
o.locations << Location.where(slug: 'latvia').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'lesotho').limit(1)[0]
o.locations << Location.where(slug: 'liberia').limit(1)[0]
o.locations << Location.where(slug: 'libya').limit(1)[0]
o.locations << Location.where(slug: 'lithuania').limit(1)[0]
o.locations << Location.where(slug: 'luxembourg').limit(1)[0]
o.locations << Location.where(slug: 'madagascar').limit(1)[0]
o.locations << Location.where(slug: 'malawi').limit(1)[0]
o.locations << Location.where(slug: 'malaysia').limit(1)[0]
o.locations << Location.where(slug: 'maldives').limit(1)[0]
o.locations << Location.where(slug: 'mali').limit(1)[0]
o.locations << Location.where(slug: 'malta').limit(1)[0]
o.locations << Location.where(slug: 'marshall_islands').limit(1)[0]
o.locations << Location.where(slug: 'mauritania').limit(1)[0]
o.locations << Location.where(slug: 'mauritius').limit(1)[0]
o.locations << Location.where(slug: 'mexico').limit(1)[0]
o.locations << Location.where(slug: 'micronesia_federated_states_of').limit(1)[0]
o.locations << Location.where(slug: 'monaco').limit(1)[0]
o.locations << Location.where(slug: 'mongolia').limit(1)[0]
o.locations << Location.where(slug: 'montenegro').limit(1)[0]
o.locations << Location.where(slug: 'morocco').limit(1)[0]
o.locations << Location.where(slug: 'mozambique').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'namibia').limit(1)[0]
o.locations << Location.where(slug: 'nauru').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'netherlands').limit(1)[0]
o.locations << Location.where(slug: 'new_zealand').limit(1)[0]
o.locations << Location.where(slug: 'nicaragua').limit(1)[0]
o.locations << Location.where(slug: 'niger').limit(1)[0]
o.locations << Location.where(slug: 'nigeria').limit(1)[0]
o.locations << Location.where(slug: 'niue').limit(1)[0]
o.locations << Location.where(slug: 'norway').limit(1)[0]
o.locations << Location.where(slug: 'oman').limit(1)[0]
o.locations << Location.where(slug: 'pakistan').limit(1)[0]
o.locations << Location.where(slug: 'palau').limit(1)[0]
o.locations << Location.where(slug: 'panama').limit(1)[0]
o.locations << Location.where(slug: 'papua_new_guinea').limit(1)[0]
o.locations << Location.where(slug: 'paraguay').limit(1)[0]
o.locations << Location.where(slug: 'peru').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'poland').limit(1)[0]
o.locations << Location.where(slug: 'portugal').limit(1)[0]
o.locations << Location.where(slug: 'qatar').limit(1)[0]
o.locations << Location.where(slug: 'republic_of_korea').limit(1)[0]
o.locations << Location.create(name: "Republic of Moldova", slug: 'republic_of_moldova', :location_type => :country)
o.locations << Location.where(slug: 'romania').limit(1)[0]
o.locations << Location.where(slug: 'russian_federation').limit(1)[0]
o.locations << Location.where(slug: 'rwanda').limit(1)[0]
o.locations << Location.where(slug: 'saint_kitts_and_nevis').limit(1)[0]
o.locations << Location.where(slug: 'saint_lucia').limit(1)[0]
o.locations << Location.where(slug: 'saint_vincent_and_the_grenadines').limit(1)[0]
o.locations << Location.where(slug: 'samoa').limit(1)[0]
o.locations << Location.create(name: "San Marino", slug: 'san_marino', :location_type => :country)
o.locations << Location.where(slug: 'sao_tome_and_principe').limit(1)[0]
o.locations << Location.where(slug: 'saudi_arabia').limit(1)[0]
o.locations << Location.where(slug: 'senegal').limit(1)[0]
o.locations << Location.where(slug: 'serbia').limit(1)[0]
o.locations << Location.where(slug: 'seychelles').limit(1)[0]
o.locations << Location.where(slug: 'sierra_leone').limit(1)[0]
o.locations << Location.where(slug: 'singapore').limit(1)[0]
o.locations << Location.where(slug: 'slovakia').limit(1)[0]
o.locations << Location.where(slug: 'slovenia').limit(1)[0]
o.locations << Location.where(slug: 'solomon_islands').limit(1)[0]
o.locations << Location.where(slug: 'somalia').limit(1)[0]
o.locations << Location.where(slug: 'south_africa').limit(1)[0]
o.locations << Location.where(slug: 'south_sudan').limit(1)[0]
o.locations << Location.where(slug: 'spain').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'sudan').limit(1)[0]
o.locations << Location.where(slug: 'suriname').limit(1)[0]
o.locations << Location.where(slug: 'sweden').limit(1)[0]
o.locations << Location.where(slug: 'switzerland').limit(1)[0]
o.locations << Location.where(slug: 'syrian_arab_republic').limit(1)[0]
o.locations << Location.where(slug: 'tajikistan').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'the_former_yugoslav_republic_of').limit(1)[0]
o.locations << Location.where(slug: 'macedonia').limit(1)[0]
o.locations << Location.where(slug: 'timorleste').limit(1)[0]
o.locations << Location.where(slug: 'togo').limit(1)[0]
o.locations << Location.where(slug: 'tonga').limit(1)[0]
o.locations << Location.where(slug: 'trinidad_and_tobago').limit(1)[0]
o.locations << Location.where(slug: 'tunisia').limit(1)[0]
o.locations << Location.where(slug: 'turkey').limit(1)[0]
o.locations << Location.where(slug: 'turkmenistan').limit(1)[0]
o.locations << Location.where(slug: 'tuvalu').limit(1)[0]
o.locations << Location.where(slug: 'uganda').limit(1)[0]
o.locations << Location.where(slug: 'ukraine').limit(1)[0]
o.locations << Location.where(slug: 'united_arab_emirates').limit(1)[0]
o.locations << Location.where(slug: 'united_kingdom').limit(1)[0]
o.locations << Location.where(slug: 'united_republic_of_tanzania').limit(1)[0]
o.locations << Location.where(slug: 'united_states_of_america').limit(1)[0]
o.locations << Location.where(slug: 'uruguay').limit(1)[0]
o.locations << Location.where(slug: 'uzbekistan').limit(1)[0]
o.locations << Location.where(slug: 'vanuatu').limit(1)[0]
o.locations << Location.create(name: "Venezuela (Bolivarian Republic of)", slug: 'venezuela_bolivarian_republic_of', :location_type => :country)
o.locations << Location.create(name: "Viet Nam", slug: 'viet_nam', :location_type => :country)
o.locations << Location.where(slug: 'yemen').limit(1)[0]
o.locations << Location.where(slug: 'zambia').limit(1)[0]
o.locations << Location.where(slug: 'zimbabwe').limit(1)[0]
o = Organization.create(name: "World Vision International", slug: "world_vision_international", when_endorsed: DateTime.new(2015, 1, 1), is_endorser: true, website: "http://www.wvi.org/")

o.sectors << Sector.where(slug: 'youth').limit(1)[0]
o.sectors << Sector.where(slug: 'education').limit(1)[0]
o.sectors << Sector.where(slug: 'emergency').limit(1)[0]
o.locations << Location.where(slug: 'bangladesh').limit(1)[0]
o.locations << Location.where(slug: 'cambodia').limit(1)[0]
o.locations << Location.where(slug: 'china').limit(1)[0]
o.locations << Location.where(slug: 'india').limit(1)[0]
o.locations << Location.where(slug: 'indonesia').limit(1)[0]
o.locations << Location.where(slug: 'laos').limit(1)[0]
o.locations << Location.where(slug: 'mongolia').limit(1)[0]
o.locations << Location.where(slug: 'myanmar').limit(1)[0]
o.locations << Location.where(slug: 'nepal').limit(1)[0]
o.locations << Location.where(slug: 'north_korea').limit(1)[0]
o.locations << Location.where(slug: 'papua_new_guinea').limit(1)[0]
o.locations << Location.where(slug: 'philippines').limit(1)[0]
o.locations << Location.where(slug: 'solomon_islands').limit(1)[0]
o.locations << Location.where(slug: 'sri_lanka').limit(1)[0]
o.locations << Location.where(slug: 'thailand').limit(1)[0]
o.locations << Location.where(slug: 'timorleste').limit(1)[0]
o.locations << Location.where(slug: 'vanuatu').limit(1)[0]
o.locations << Location.where(slug: 'vietnam').limit(1)[0]
o.locations << Location.where(slug: 'albania').limit(1)[0]
o.locations << Location.where(slug: 'bosnia_and_herzegovina').limit(1)[0]
o.locations << Location.where(slug: 'kosovo').limit(1)[0]
o.locations << Location.where(slug: 'romania').limit(1)[0]
o.locations << Location.where(slug: 'georgia').limit(1)[0]
o.locations << Location.where(slug: 'armenia').limit(1)[0]
o.locations << Location.where(slug: 'syria').limit(1)[0]
o.locations << Location.where(slug: 'lebanon').limit(1)[0]
o.locations << Location.where(slug: 'jordan').limit(1)[0]
o.locations << Location.where(slug: 'turkey').limit(1)[0]
o.locations << Location.create(name: "Gaza", slug: 'gaza', :location_type => :country)
o.locations << Location.where(slug: 'iraq').limit(1)[0]
o.locations << Location.where(slug: 'kenya').limit(1)[0]

f = File.join(Rails.root, 'db', 'contacts.rb')
if File.exists?(f)
  load f
end
