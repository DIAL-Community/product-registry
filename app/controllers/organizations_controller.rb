...
class OrganizationsController < ApplicationController
  ...
  def index
    if params[:without_paging]
      ...
      !params[:sector_id].nil? && @organizations = @organizations.joins(:sectors).where('sectors.id = ?',
                                                                                        params[:sector_id])
  ...
  def update
    ...
    if params[:selected_contacts].present?
      @organization.organizations_contacts.each do |organization_contact|
        ...
        organization_contact.update(ended_at: Time.now.utc) unless params[:selected_contacts].include?(contact_id.to_s)
      end
      ...
      params[:selected_contacts].each do |selected_contact|
        ...
        next if organization_contact && organization_contact.ended_at.nil?
    ...
    if organization_params[:organization_description].present?
      @organization_description = OrganizationDescription.new if @organization_description.locale != I18n.locale.to_s
      @organization_description.organization_id = @organization.id
      @organization_description.locale = I18n.locale
      @organization_description.description = organization_params[:organization_description]
      @organization_description.save
    ...
    respond_to do |format|
      if @organization.errors.none?
        @organization.update(organization_params) if organization_params
        format.html do
  ...
  def agg_capabilities
    ...
    operator_services.each do |operator|
      ...
      next if curr_capabilities.empty?
  ...
  def set_organization
    ...
    @organization = Organization.find(params[:id]) if @organization.nil? && params[:id].scan(/\D/).empty?

    @organization_description = OrganizationDescription.where(organization_id: @organization, locale: I18n.locale)
                                                       .first
    @organization_description ||= OrganizationDescription.where(organization_id: @organization,
  ...
  def geocode(magic_key, auth_token)
    ...
    convert_location_using_google(points, google_auth_key)
  end

  def convert_location_using_google(points, google_auth_key)
    # Location is a point:
    ...
    office_data['city'] = office_data['admin_city'] if office_data['city'].blank?
    ...
    if office_data['city'].blank? || office_data['region'].blank? || office_data['country_code'].blank?
      office.latitude = points[0].x
      office.longitude = points[0].y
      office.city = 'Unknown'
      office.city = office_data['city'] unless office_data['city'].blank?
    end
  ...
  def set_core_services
    @core_services = %w[Airtime API HS Mobile-Internet Mobile-Money Ops-Maintenance OTT SLA
  ...
  def organization_params
    ...
    permitted_attributes = policy(@organization).permitted_attributes unless @organization.nil?
