...
module BuildingBlocksHelper
  def bb_footer(building_block, category)
    ...
    when 'products'
      building_block.products.each do |product|
        ...
        image = Hash[filename: product.image_file, tooltip: tooltip, id: product.slug,
                     alt_text: t('alt.el-logo', el: "#{product.name} #{t('model.product')}").humanize,
                     controller: 'products']
        images.push(image)
      end
    when 'workflows'
      building_block.workflows.each do |workflow|
        ...
        image = Hash[filename: workflow.image_file, tooltip: tooltip, id: workflow.slug,
                     alt_text: t('alt.el-logo', el: "#{workflow.name} #{t('model.workflow')}").humanize,
  ...
  def bb_footer_popover(elements, title)
    content = "<div class=\"border rounded bg-secondary text-white clearfix border card-header\">" \
              "#{t(title, count: elements.count)}</div><div>#{bb_format_image_popover(elements)}</div>"
    content.html_safe
  end
